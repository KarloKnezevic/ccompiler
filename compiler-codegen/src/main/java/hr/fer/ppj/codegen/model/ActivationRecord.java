 ...
 *
 * <p>This class manages the layout of local variables and parameters on the stack for a single
 * function. It implements the <b>stack frame allocation algorithm</b> used by the FRISC calling
 * convention, tracking the offsets of variables relative to the frame pointer (R5) and providing
 * methods for allocating space for new variables.
 *
 ...
 *
 * <ol>
 *   <li><b>Parameter Allocation:</b> Parameters are allocated first, starting at R5+8. Each
 *       parameter takes 4 bytes (32 bits), regardless of type (int, char, pointer). Parameters are
 *       allocated in declaration order (left-to-right).
 *   <li><b>Local Variable Allocation:</b> Local variables are allocated after parameters, starting
 *       at R5-4 and growing downward (negative offsets). Each variable's size is rounded up to a
 *       multiple of 4 bytes for stack alignment.
 *   <li><b>Array Allocation:</b> Arrays are allocated as contiguous blocks. For example, {@code int
 *       a[5]} uses 20 bytes (5 * 4) at offsets R5-20 to R5-1.
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>Frame Pointer (R5):</b> Points to the saved old R5 value. This is fixed during function
 *       execution and provides a stable reference point for accessing parameters and local
 *       variables.
 *   <li><b>Parameter Offsets:</b> Always positive (R5+8, R5+12, etc.). The first parameter is at
 *       R5+8 because R5+0 contains the saved old R5 and R5+4 contains the return address.
 *   <li><b>Local Variable Offsets:</b> Always negative (R5-4, R5-8, etc.). Local variables are
 *       allocated downward from R5 to allow for variable-sized arrays and dynamic allocation.
 *   <li><b>Stack Alignment:</b> All allocations are rounded up to multiples of 4 bytes to maintain
 *       4-byte alignment, which is required for efficient memory access on FRISC.
 * </ul>
 *
 * <p><b>FRISC Calling Convention Details:</b>
 *
 * <ul>
 *   <li><b>Parameter Passing:</b> Arguments are pushed on the stack by the caller in right-to-left
 *       order (C convention). The callee accesses them via positive offsets from R5.
 *   <li><b>Return Value:</b> Returned in register R6, not on the stack.
 *   <li><b>Stack Pointer (R7):</b> Points to the top of the stack (lowest allocated address). It is
 *       decremented when allocating local variables and incremented when deallocating.
 *   <li><b>Frame Pointer (R5):</b> Set in the function prologue by saving the old R5 and setting R5
 *       = R7. Restored in the epilogue before returning.
 * </ul>
 *
 * <p><b>Complexity:</b>
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) for addParameter() and addLocalVariable() operations
 *   <li><b>Space Complexity:</b> O(n) where n is the number of variables (parameters + locals)
 * </ul>
 *
 * <p><b>Usage Example:</b>
 *
...
public final class ActivationRecord {
  ...
  /**
   * Map from variable name to its stack offset relative to R5 (frame pointer). Positive offsets are
   * for parameters (R5+8, R5+12, etc.), negative offsets are for local variables (R5-4, R5-8,
   * etc.).
   */
  private final Map<String, Integer> variableOffsets = new HashMap<>();

  /**
   * Map from variable name to its size in bytes. Used to determine if a variable is an array (size
   * > 4 bytes).
   */
  private final Map<String, Integer> variableSizes = new HashMap<>();

  /**
   * Current offset for the next parameter (positive, relative to R5). First parameter starts at
   * R5+8 (after old R5 at +0 and return address at +4).
   */
   ...
   *
   * <p>Parameters are added in declaration order and assigned offsets starting at R5+8. Each
   * parameter takes 4 bytes (32 bits) regardless of type.
   *
   ...
   *
   * <p>Parameters are added in declaration order and assigned offsets starting at R5+8. For struct
   * parameters, the size is the struct size in bytes.
   *
   * @param name the parameter name
   * @param sizeInBytes the size of the parameter in bytes (4 for scalars, sizeof(struct) for
   *     structs)
   * @return the stack offset for this parameter (positive, relative to R5)
   */
   ...
   *
   * <p>Local variables are allocated with negative offsets from R5, starting at R5-4. The size is
   * rounded up to a multiple of 4 bytes for alignment.
   *
   * <p><b>FRISC Stack Layout:</b>
   *
   * <ul>
   *   <li>First local variable: R5-4
   *   <li>Second local variable: R5-8
   *   <li>Arrays: Allocated as contiguous block (e.g., int a[5] uses 20 bytes at R5-20 to R5-1)
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>Size is aligned to 4-byte boundary (rounds up)
   *   <li>Arrays: size = num_elements * 4 (element size is 4 bytes for both int and char)
   *   <li>Stack grows downward: R7 decreases by aligned size
   * </ul>
   *
   * <p>The size is rounded up to a multiple of 4 bytes for stack alignment. Local variables are
   * allocated downward from R5 (negative offsets).
   *
   * @param name the variable name
   ...
   *
   * <p>Parameters have positive offsets (R5+8, R5+12, etc.), while local variables have negative
   * offsets (R5-4, R5-8, etc.).
   *
   * @param name the variable name
   ...
   *
   * <p>This is used to determine how much stack space to allocate/deallocate in the function
   ...
   *
   * <p>Parameters are accessed with positive offsets (e.g., "(R5+08)"), while local variables are
   * accessed with negative offsets (e.g., "(R5-04)"). All offsets are formatted as hexadecimal.
   *
   * @param name the variable name
   ...
   *
   * <p>This can be used to determine if a variable is an array: if size > 4 bytes, it's likely an
   * array.
   *
   * @param name the variable name
   ...
   *
   * <p>Stack offsets (addresses) use hex format: "08", "0C", "-04", etc. This ensures consistency
   * with FRISC assembly conventions where addresses and offsets are represented in hexadecimal.
   *
