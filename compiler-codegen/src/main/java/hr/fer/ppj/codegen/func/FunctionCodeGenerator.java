 ...
 *
 * <p>This class serves as the main coordinator for function code generation, delegating to
 * specialized classes for:
 *
 * <ul>
 *   <li>Function information extraction ({@link FunctionInfoExtractor})
 *   <li>Prologue/epilogue generation ({@link FunctionPrologueEpilogueGenerator})
 *   <li>Statement generation ({@link StatementCodeGenerator})
 * </ul>
 *
 * <p><b>Grammar Rule:</b> Processes {@code <prijevodna_jedinica>} to find {@code
 * <definicija_funkcije>} nodes:
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>Arguments are passed on the stack (right-to-left)
 *   <li>Return values are placed in register R6
 *   <li>Caller cleans up arguments from the stack
 *   <li>Local variables are allocated on the stack
 * </ul>
 *
 * <p>Each function is translated to a FRISC subroutine with a label following the pattern {@code
 * F_<FUNCTION_NAME>}. The subroutine manages its own activation record (stack frame) for parameters
 * and local variables.
 *
...
public final class FunctionCodeGenerator {
   ...
   *
   * <p>This method traverses the parse tree looking for function definitions and generates the
   * corresponding FRISC subroutines.
   *
   ...
   * </pre>
   *
   * <p>Extracts function name, parameters, and body from the parse tree structure, then delegates
   * to {@link #generateFunction(String, NonTerminalNode, NonTerminalNode)}.
   *
   ...
   *
   * <ul>
   *   <li>Function label: {@code F_<function_name>}
   *   <li>Prologue: Save R5, set R5 = R7, allocate locals
   *   <li>Parameters: At positive offsets from R5 (R5+8, R5+12, ...)
   *   <li>Locals: At negative offsets from R5 (R5-4, R5-8, ...)
   *   <li>Return value: In R6
   *   <li>Epilogue: Deallocate locals, restore R5, RET
   * </ul>
   *
  ...
  private void generateFunction(
          ...
          if (strippedParamType instanceof StructType structType) {
            ...
            if (infoExtractor.getArraySizeExtractor() != null) {
              ...
              NestedStructArraySizeExtractor.extractNestedStructArraySizes(
                  structType, infoExtractor.getArraySizeExtractor(), nestedStructArraySizes);
            }
    ...
    int elementSize = 4;
    List<FunctionInfoExtractor.VariableInfo> localVars =
        infoExtractor.extractLocalVariables(body, elementSize);
    ...
    CodeGenContext functionContext =
        context.withActivationRecord(activationRecord).withFunctionExitLabel(exitLabel);
