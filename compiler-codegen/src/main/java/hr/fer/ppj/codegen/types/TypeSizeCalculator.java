 ...
 *
 * <p>This class provides methods to compute the memory size of all supported types, including
 * primitive types, pointers, arrays, and structs. It implements the <b>type size calculation
 * algorithm</b> used throughout code generation.
 *
 * <p><b>Type Sizes:</b>
 *
 * <ul>
 *   <li><b>char:</b> 4 bytes (same as int and float in FRISC)
 *   <li><b>int:</b> 4 bytes
 *   <li><b>float:</b> 4 bytes (Q16.16 fixed-point format)
 *   <li><b>pointer:</b> 4 bytes
 *   <li><b>array T[n]:</b> n × sizeof(T) bytes (size must be provided from semantic attributes)
 *   <li><b>struct S:</b> SUM(sizeof(field)) for all fields (delegates to {@link
 *       StructSizeCalculator})
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li><b>Type Classification:</b> Determine the type kind (primitive, pointer, array, struct)
 *   <li><b>Size Lookup/Calculation:</b>
 *       <ul>
 *         <li>Primitive types: Fixed size (char/int/float = 4 bytes)
 *         <li>Pointer types: Fixed size (4 bytes)
 *         <li>Array types: Cannot calculate from type alone (size must be provided)
 *         <li>Struct types: Recursively calculate using {@link StructSizeCalculator}
 *       </ul>
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>Simple structs:</b> Sum of all field sizes
 *   <li><b>Structs with arrays:</b> Requires array sizes to be provided (extracted from parse tree)
 *   <li><b>Nested structs:</b> Recursively calculates nested struct sizes
 *   <li><b>Nested structs with arrays:</b> Requires array sizes for all nested structs at all
 *       levels
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Key: Struct tag name (e.g., "Inner")
 *   <li>Value: Map from field name to array length (e.g., {"arr": 10})
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Array Types:</b> ArrayType doesn't store size information. Array sizes must be provided
 *       from semantic attributes or extracted from the parse tree.
 *   <li><b>Nested Structs:</b> For structs containing arrays, array sizes must be provided via the
 *       nestedStructArraySizes parameter.
 * </ul>
 *
 * <p><b>Example:</b>
 *
 ...
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) for primitive/pointer types, O(n) for struct types where n is
...
public final class TypeSizeCalculator {
   ...
   *
   * <ul>
   *   <li>Primitive types: char (4), int (4), float (4)
   *   <li>Pointer types: 4 bytes
   *   <li>Array types: throws IllegalArgumentException (size must be provided)
   *   <li>Struct types: recursively calculated via {@link StructSizeCalculator}
   * </ul>
   *
   * @param type the type to calculate size for
   * @return the size in bytes
   * @throws NullPointerException if type is null
   * @throws IllegalArgumentException if type is not supported or is an array (size must be
   *     provided)
   */
   ...
   *
   * <p>This overload allows providing array sizes for nested structs that contain arrays. The map
   * should map struct tag names to their array sizes (field name -> array length).
   *
   * <p><b>Note:</b> This method still cannot calculate array sizes from ArrayType alone. Array
   * sizes must be provided from semantic attributes or extracted from the parse tree.
   *
   * @param type the type to calculate size for
   ...
   *     provided)
   */
  public static int calculateTypeSize(
      Type type, Map<String, Map<String, Integer>> nestedStructArraySizes) {
    ...
    // Handle array types
      ...
      throw new IllegalArgumentException(
          "Cannot calculate array size from ArrayType alone - size must be provided from semantic attributes");
    }
    // Handle struct types (delegates to StructSizeCalculator)
      ...
      return StructSizeCalculator.calculateStructSize(
          structType, arraySizes, nestedStructArraySizes);
    }
    // Unsupported type
    else {
      throw new IllegalArgumentException(
          "Unsupported type for size calculation: " + stripped.getClass().getSimpleName());
    }
  }
}
