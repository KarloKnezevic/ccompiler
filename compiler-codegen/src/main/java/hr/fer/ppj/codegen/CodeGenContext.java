 ...
 * generation process. It serves as a <b>context parameter</b> that is passed down through the
 * recursive tree traversal, allowing all code generation methods to access shared infrastructure
 ...
 *
 * <ul>
 *   <li><b>Thread Safety:</b> Immutability ensures the context can be safely shared across multiple
 *       code generation passes or threads
 *   <li><b>Predictable State:</b> Since the context cannot be modified, there are no hidden side
 *       effects from context mutations
 *   <li><b>Functional Style:</b> The {@code with*} methods create new contexts, following a
 *       functional programming style that makes state transitions explicit
 *   <li><b>Recursive Safety:</b> When traversing the parse tree recursively, each level can create
 *       a new context without affecting parent contexts
 * </ul>
 *
 * <p><b>Context Components:</b>
 *
 * <ul>
 *   <li><b>{@code globalScope}:</b> Global symbol table from semantic analysis. Contains all global
 *       variables, functions, and type definitions. Used to resolve global identifiers during code
 *       generation.
 *   <li><b>{@code emitter}:</b> FRISC code emitter for instruction output. All generated FRISC
 *       instructions are emitted through this object. The emitter buffers code in memory until
 *       {@code writeToFile()} is called.
 *   <li><b>{@code labelGenerator}:</b> Generator for unique labels (L1, L2, F_MAIN, etc.). Ensures
 *       all generated labels are unique by maintaining separate counters for different label types
 *       (functions, control flow, loops, etc.).
 *   <li><b>{@code activationRecord}:</b> Current function's stack frame (null for global scope).
 *       Tracks local variables and parameters with their stack offsets relative to R5. Used to
 *       generate correct memory addresses for local variable access.
 *   <li><b>{@code functionExitLabel}:</b> Label for function epilogue (null if not in function).
 *       Used by return statements to jump to the function exit code. Set when entering a function,
 *       cleared when exiting.
 *   <li><b>{@code loopBreakLabel}:</b> Label for break statements (null if not in loop). Used by
 *       break statements to exit the innermost loop. Set when entering a loop (while, for), cleared
 *       when exiting.
 *   <li><b>{@code loopContinueLabel}:</b> Label for continue statements (null if not in loop). Used
 *       by continue statements to jump to the loop condition check. Set when entering a loop,
 *       cleared when exiting.
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li><b>Initial Context:</b> Created at the start of code generation with global scope only (no
 *       activation record, no function/loop labels)
 *   <li><b>Function Entry:</b> When entering a function, a new context is created with: - A new
 *       activation record for the function's stack frame - A function exit label for return
 *       statements
 *   <li><b>Loop Entry:</b> When entering a loop, a new context is created with: - Loop break and
 *       continue labels - Preserves the function context (activation record, exit label)
 *   <li><b>Context Exit:</b> When exiting a function or loop, the previous context is restored (by
 ...
 *
 * <ul>
 *   <li><b>Local Variable Offsets:</b> Negative offsets from R5 (e.g., R5-4, R5-8)
 *   <li><b>Parameter Offsets:</b> Positive offsets from R5 (e.g., R5+8, R5+12)
 *   <li><b>Total Stack Frame Size:</b> Used to allocate/deallocate stack space in prologue/epilogue
 *   <li><b>Array Variable Sizes:</b> Used to detect array parameters (size > 4 bytes)
 * </ul>
 *
 * <p><b>Usage Example:</b>
 *
