 ...
 *
 * <p>FRISC architecture does not have a native MUL instruction, so multiplication is implemented
 * using the <b>binary shift-and-add algorithm</b> (also known as the <b>Russian peasant
 * algorithm</b> or <b>binary multiplication</b>).
 *
 * <p><b>Algorithm: Binary Shift-and-Add Multiplication (Russian Peasant Algorithm)</b>
 *
 * <p>This algorithm multiplies two integers by examining each bit of the multiplier and
 * conditionally adding shifted versions of the multiplicand. The algorithm works as follows:
 *
 * <ol>
 ...
 *   <li><b>Initialization:</b> Set accumulator to 0
 *   <li><b>Binary Loop:</b> While multiplier (b) is not zero:
 *       <ul>
 *         <li>If the least significant bit of b is set (b & 1 == 1), add the current value of a to
 *             the accumulator
 *         <li>Shift a left by 1 bit (a <<= 1), effectively multiplying by 2
 *         <li>Shift b right by 1 bit (b >>= 1), effectively dividing by 2
 *       </ul>
 *   <li><b>Sign Application:</b> If the original sign was negative, negate the result
 * </ol>
 *
 * <p><b>Mathematical Basis:</b>
 *
 * <p>The algorithm is based on the binary representation of numbers. For example, multiplying 13 ×
 * 7:
 *
 * <pre>
 * 13 = 1101₂ (binary)
 ...
 *
 * <ul>
 *   <li>When bit 0 of 7 is set: add 13 × 1 = 13
 *   <li>When bit 1 of 7 is set: add 13 × 2 = 26
 *   <li>When bit 2 of 7 is set: add 13 × 4 = 52
 * </ul>
 *
 * <p><b>Complexity Analysis:</b>
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(log₂|b|) ≈ O(32) for 32-bit integers. The loop iterates at most
 *       32 times (once per bit in the multiplier).
 *   <li><b>Space Complexity:</b> O(1) - uses only a few registers
 *   <li><b>Comparison with Naive Algorithm:</b> The naive algorithm (repeated addition) would be
 *       O(|b|), which is O(2³²) in the worst case. This algorithm is exponentially faster.
 * </ul>
 *
 * <p><b>Edge Cases Handled:</b>
 *
 * <ul>
 *   <li><b>Zero Multiplier:</b> If b == 0, return 0 immediately (early exit optimization)
 *   <li><b>Negative Operands:</b> Convert to positive, track sign, apply sign to result
 *   <li><b>Overflow:</b> 32-bit signed multiplication can overflow, but the algorithm correctly
 *       computes the low 32 bits of the product (C standard behavior)
 * </ul>
 *
 * <p><b>FRISC Register Usage:</b>
 *
 * <ul>
 *   <li><b>R0:</b> Multiplicand (a), shifted left each iteration
 *   <li><b>R1:</b> Multiplier (b), shifted right each iteration
 *   <li><b>R2:</b> Accumulator (result being built)
 *   <li><b>R3:</b> Temporary (used for bit test: b & 1)
 *   <li><b>R4:</b> Sign flag (0 = positive, 1 = negative)
 *   <li><b>R6:</b> Return value (final result)
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Efficiency:</b> O(log n) instead of O(n) for naive repeated addition
 *   <li><b>Simplicity:</b> Easy to implement in assembly with basic shift and add operations
 *   <li><b>Correctness:</b> Handles all edge cases (zero, negative, overflow)
 *   <li><b>No Hardware Dependency:</b> Works on any architecture with shift and add
 * </ul>
 *
...
public final class MultiplicationHelperGenerator {
  ...
  public void generate(CodeGenContext context) {
    ...
    emitter.emitInstruction(
        "LOAD", "R0", "(R5+08)", "a (left operand, pushed last, first parameter)");
    emitter.emitInstruction(
        "LOAD", "R1", "(R5+0C)", "b (right operand, pushed first, second parameter)");
