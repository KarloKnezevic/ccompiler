 ...
 * 
 * <p>This class handles the generation of code for all floating-point operations by delegating
 * to specialized helper functions. Float operations use <b>Q16.16 fixed-point representation</b>
 * and call helper functions (F_FADD, F_FSUB, F_FMUL, F_FDIV, F_FCMP) for arithmetic and comparison.
 * 
 ...
 *       <ul>
 *         <li>Evaluate left operand (may be int, char, or float)</li>
 *         <li>If operand is not float, convert to float using F_I2F</li>
 *         <li>Save left operand (now in Q16.16 format) on stack</li>
 *         <li>Evaluate right operand (may be int, char, or float)</li>
 *         <li>If operand is not float, convert to float using F_I2F</li>
 *         <li>Restore operands to R0 and R1 (both in Q16.16 format)</li>
 *       </ul>
 *   </li>
 *   <li><b>Helper Function Call:</b>
 *       <ul>
 *         <li>Push arguments on stack (right-to-left order)</li>
 *         <li>Call the appropriate float helper function</li>
 *         <li>Clean up arguments from stack</li>
 *         <li>Move return value from R6 to R0</li>
 *       </ul>
 *   </li>
 *   <li><b>Result Processing:</b>
 *       <ul>
 *         <li>For comparisons: Convert three-way comparison result (-1, 0, 1) to boolean (0, 1)</li>
 *         <li>For arithmetic: Result is already in Q16.16 format in R0</li>
 *       </ul>
 *   </li>
 * </ol>
 * 
 ...
 *   <li>Actual value = stored_integer / 65536.0</li>
 * </ul>
 * 
 * <p><b>Type Conversion (Usual Arithmetic Conversions):</b>
 * 
 * <p>C standard specifies that when an integer operand is used with a float operand,
 * the integer is automatically promoted to float. This is implemented by:
 * <ul>
 *   <li>Checking the type of each operand</li>
 *   <li>If operand is int or char, calling F_I2F to convert to Q16.16</li>
 *   <li>If operand is already float, no conversion is needed</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li><b>== (OP_EQ):</b> result == 0 → 1, else → 0</li>
 *   <li><b>!= (OP_NEQ):</b> result != 0 → 1, else → 0</li>
 *   <li><b>&lt; (OP_LT):</b> result < 0 → 1, else → 0</li>
 *   <li><b>&gt; (OP_GT):</b> result > 0 → 1, else → 0</li>
 *   <li><b>&lt;= (OP_LTE):</b> result <= 0 → 1, else → 0</li>
 *   <li><b>&gt;= (OP_GTE):</b> result >= 0 → 1, else → 0</li>
 * </ul>
 * 
 * <p><b>FRISC Register Usage:</b>
 * <ul>
 *   <li><b>R0:</b> Left operand (after conversion), then result</li>
 *   <li><b>R1:</b> Right operand (after conversion)</li>
 *   <li><b>R6:</b> Return value from helper functions</li>
 ...
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) for code generation, but actual runtime depends on
 *       the helper function complexity (O(1) for add/sub, O(32) for mul, O(64) for div)</li>
 *   <li><b>Space Complexity:</b> O(1) - uses only registers and temporary stack space</li>
 * </ul>
 * 
...
public final class FloatOperationGenerator {
     ...
     */
    public FloatOperationGenerator(CodeGenContext context, ExpressionCodeGenerator expressionGenerator) {
     ...
     */
    public void generateAdditiveOperation(NonTerminalNode left, NonTerminalNode right, String operation) {
