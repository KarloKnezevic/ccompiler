 ...
 * 
 * <p>This class encapsulates the common pattern of evaluating left and right
 * operands for binary operations, including stack management and type conversion.
 * It implements the <b>operand evaluation pattern</b> used by all binary expression
 * generators, eliminating code duplication and ensuring consistent operand handling.
 * 
 ...
 * <ul>
 *   <li><b>Standard Evaluation:</b> {@code evaluateOperands()} - for integer operations</li>
 *   <li><b>Float Evaluation:</b> {@code evaluateOperandsWithFloatConversion()} - for float
 *       operations with automatic int-to-float conversion</li>
 * </ul>
 * 
 ...
 * <ol>
 *   <li><b>Evaluate Left Operand:</b> Recursively generate code for the left expression.
 *       The result is left in R0. This may involve complex nested expressions, function
 *       calls, or simple constants.</li>
 *   <li><b>Save Left Operand:</b> Push R0 to the stack to preserve it. This is necessary
 *       because evaluating the right operand will overwrite R0.</li>
 *   <li><b>Evaluate Right Operand:</b> Recursively generate code for the right expression.
 *       The result is left in R0, overwriting the left operand value that was there.</li>
 *   <li><b>Restore Operands:</b> Move R0 (right operand) to R1, then pop the left operand
 *       back to R0. After this step:
 *       <ul>
 *         <li>R0 contains the left operand</li>
 *         <li>R1 contains the right operand</li>
 *       </ul>
 *   </li>
 * </ol>
 * 
 ...
 * <ul>
 *   <li><b>Register Scarcity:</b> FRISC has limited registers (R0-R7), and R0 is the
 *       standard result register. We cannot keep both operands in registers simultaneously
 *       during right operand evaluation.</li>
 *   <li><b>Recursive Evaluation:</b> Right operand evaluation may itself be a complex
 *       expression that uses R0, so we must save the left operand before evaluating right.</li>
 *   <li><b>Stack as Temporary Storage:</b> The stack provides a convenient temporary storage
 *       location that is automatically managed and doesn't interfere with register allocation.</li>
 * </ul>
 * 
 ...
 * 
 * <p>The {@code evaluateOperandsWithFloatConversion()} method extends the standard pattern
 * with automatic type conversion:
 * <ol>
 *   <li>Evaluate left operand</li>
 *   <li>If left is not float, convert int/char to float using F_I2F helper</li>
 *   <li>Save left operand (now in float format)</li>
 *   <li>Evaluate right operand</li>
 *   <li>If right is not float, convert int/char to float using F_I2F helper</li>
 *   <li>Restore operands (both now in float format)</li>
 * </ol>
 * 
 ...
 * <ul>
 *   <li><b>Push Before Right Evaluation:</b> Left operand is pushed before right evaluation
 *       to preserve it</li>
 *   <li><b>Pop After Right Evaluation:</b> Left operand is popped after right evaluation
 *       to restore it</li>
 *   <li><b>Stack Balance:</b> Each push is matched with exactly one pop, maintaining
 *       stack balance</li>
 * </ul>
 * 
 * <p><b>Register Usage:</b>
 * <ul>
 *   <li><b>R0:</b> Expression result register (left operand after restoration)</li>
 *   <li><b>R1:</b> Right operand register</li>
 *   <li><b>Stack:</b> Temporary storage for left operand during right evaluation</li>
 * </ul>
 * 
 * <p><b>Complexity:</b>
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) for the evaluation pattern itself, but the actual
 *       time depends on the complexity of the operand expressions</li>
 *   <li><b>Space Complexity:</b> O(1) stack space (one 4-byte value pushed/popped)</li>
 * </ul>
 * 
...
public final class OperandEvaluator {
     ...
     * 
     * <p>This method implements the standard operand evaluation pattern used by all
     * binary operations. It ensures that after evaluation:
     * <ul>
     *   <li>R0 contains the left operand</li>
     *   <li>R1 contains the right operand</li>
     * </ul>
     ...
     * <p><b>Preconditions:</b>
     * <ul>
     *   <li>Both operands must be valid expression nodes</li>
     *   <li>Stack must have sufficient space for one 4-byte push</li>
     * </ul>
     * 
     * <p><b>Postconditions:</b>
     * <ul>
     *   <li>R0 contains the left operand value</li>
     *   <li>R1 contains the right operand value</li>
     *   <li>Stack is balanced (no net change in stack pointer)</li>
     * </ul>
     * 
     * <p><b>Side Effects:</b>
     * <ul>
     *   <li>Emits FRISC instructions to evaluate both operands</li>
     *   <li>Temporarily uses stack space (one 4-byte push/pop)</li>
     *   <li>May use other registers during operand evaluation (R0, R1, etc.)</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method extends the standard operand evaluation pattern with automatic
     * type conversion for float operations. It implements C's <b>usual arithmetic
     * conversions</b>, where integer operands are automatically promoted to float
     * when mixed with float operands.
     * 
     * <p><b>Type Conversion Rules:</b>
     * <ul>
     *   <li>If an operand is already float, no conversion is needed</li>
     *   <li>If an operand is int or char, it is converted to float using F_I2F helper</li>
     *   <li>After conversion, both operands are in Q16.16 fixed-point format</li>
     * </ul>
     * 
     * <p><b>Algorithm:</b>
     * <ol>
     *   <li>Evaluate left operand (result in R0)</li>
     *   <li>Check left operand type. If not float, convert int/char to float using F_I2F</li>
     *   <li>Save left operand (now in float format) on stack</li>
     *   <li>Evaluate right operand (result in R0)</li>
     *   <li>Check right operand type. If not float, convert int/char to float using F_I2F</li>
     *   <li>Move right operand to R1, restore left operand to R0</li>
     * </ol>
     * 
     * <p><b>Postconditions:</b>
     * <ul>
     *   <li>R0 contains the left operand (as float, Q16.16 format)</li>
     *   <li>R1 contains the right operand (as float, Q16.16 format)</li>
     *   <li>Both operands are in the same format, ready for float operations</li>
     * </ul>
     * 
     ...
     *   <li>16 low bits: fractional part (scaled by 65536)</li>
     *   <li>Example: 1.5 is represented as 98304 (1 * 65536 + 0.5 * 65536)</li>
     * </ul>
     * 
     * <p><b>Side Effects:</b>
     * <ul>
     *   <li>Emits FRISC instructions to evaluate both operands</li>
     *   <li>May call F_I2F helper function for type conversion</li>
     *   <li>Marks F_I2F as needed if conversion is performed</li>
     *   <li>Uses stack space temporarily (one 4-byte push/pop)</li>
     * </ul>
     * 
     * @param left the left operand expression node (must not be null)
     * @param right the right operand expression node (must not be null)
     ...
     * 
     * <p>This method calls the F_I2F helper function to convert a 32-bit signed integer
     * to Q16.16 fixed-point format. The conversion multiplies the integer by 65536
     * (2^16) to scale it into the fixed-point representation.
     * 
     * <p><b>Conversion Algorithm:</b>
     * <ul>
     *   <li>Input: 32-bit signed integer in R0</li>
     *   <li>Process: Multiply by 65536 (left shift by 16 bits)</li>
     *   <li>Output: Q16.16 fixed-point value in R0</li>
     * </ul>
     * 
     * <p><b>Example:</b>
     * <ul>
     *   <li>Integer 1 → Float 65536 (1.0 in Q16.16)</li>
     *   <li>Integer 2 → Float 131072 (2.0 in Q16.16)</li>
     *   <li>Integer -1 → Float -65536 (-1.0 in Q16.16)</li>
     * </ul>
     * 
     * <p><b>FRISC Calling Convention:</b>
     * <ol>
     *   <li>Mark F_I2F as needed (ensures helper is generated)</li>
     *   <li>Push integer argument on stack</li>
     *   <li>Call F_I2F helper function</li>
     *   <li>Clean up stack (remove 4-byte argument)</li>
     *   <li>Move return value from R6 to R0</li>
     * </ol>
     * 
     * <p><b>Preconditions:</b>
     * <ul>
     *   <li>R0 contains a 32-bit signed integer value</li>
     * </ul>
     * 
     * <p><b>Postconditions:</b>
     * <ul>
     *   <li>R0 contains the float value in Q16.16 format</li>
     *   <li>Stack is balanced (argument pushed and cleaned up)</li>
     * </ul>
     * 
     * <p><b>Side Effects:</b>
     * <ul>
     *   <li>Marks F_I2F helper as needed</li>
     *   <li>Emits FRISC instructions for the conversion</li>
     *   <li>Temporarily uses stack space (one 4-byte push)</li>
     * </ul>
     */
