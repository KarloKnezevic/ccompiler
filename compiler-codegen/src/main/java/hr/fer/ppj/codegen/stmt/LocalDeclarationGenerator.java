 ...
 *
 * <p>This class handles the generation of code for local variable declarations within compound
 * statements (blocks), including:
 *
 * <ul>
 *   <li>Simple variable declarations: {@code int x;}
 *   <li>Variable declarations with initializers: {@code int x = 5;}
 *   <li>Array declarations: {@code int a[5];}
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Local variables allocated in activation record (stack frame)
 *   <li>Variables accessed via negative offsets from R5 (frame pointer)
 *   <li>Initializers evaluated and stored in variable location
 *   <li>Array size extracted from parse tree (defaults to 4 bytes per element)
 * </ul>
 *
...
public final class LocalDeclarationGenerator {
  ...
  private void processInitDeclarator(NonTerminalNode node) {
    ...
    if (varName != null && context.activationRecord() != null) {
      ...
      if (children.size() == 1) {
        ...
        context
            .emitter()
            .emitComment("Local variable " + varName + " at " + address + " (no initializer)");
      } else if (children.size() == 3) {
        // Declarator with initializer: <deklarator> OP_ASSIGN <inicijalizator>
        ...
        context
            .emitter()
            .emitComment("Local variable " + varName + " at " + address + " with initializer");
   ...
   *
   * <p>This method tries to get type from semantic attributes first, then falls back to extracting
   * array size from the parse tree.
   *
  ...
  private boolean isArrayInitializerList(NonTerminalNode initializer) {
    ...
    if (children.size() >= 3) {
      ...
      if (first instanceof TerminalNode t1
          && "L_VIT_ZAGRADA".equals(t1.symbol())
          && last instanceof TerminalNode t2
          && "D_VIT_ZAGRADA".equals(t2.symbol())) {
        return true;
      }
    }
    return false;
