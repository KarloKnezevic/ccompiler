 ...
 * 
 * <p>This class handles the critical distinction between different parameter types
 * in C's calling convention:
 * <ul>
 *   <li><b>Array/Pointer Parameters:</b> Decay to pointers, stored as 4-byte pointer values.
 *       Need LOAD to get the actual base address.</li>
 *   <li><b>Struct-by-Value Parameters:</b> Stored directly on the stack (multiple words).
 *       Do NOT need LOAD - the address is already the struct address.</li>
 *   <li><b>Scalar Parameters:</b> Stored directly on the stack (4 bytes).
 *       Do NOT need LOAD when computing address.</li>
 * </ul>
 * 
 * <p><b>Purpose:</b>
 * 
 * <p>When computing addresses for array indexing (e.g., {@code a[i]} where {@code a} is a parameter),
 * we need to know whether to emit a LOAD instruction:
 * <ul>
 *   <li><b>Array/Pointer Parameters:</b> Parameter slot contains pointer value → need LOAD</li>
 *   <li><b>Struct Parameters:</b> Parameter slot contains struct bytes directly → no LOAD</li>
 *   <li><b>Scalar Parameters:</b> Parameter slot contains value directly → no LOAD</li>
 * </ul>
 * 
 * <p><b>Algorithm:</b>
 * 
 ...
 * <ol>
 *   <li><b>Size Check:</b> If parameter size > 4 bytes, it's a struct-by-value → no LOAD</li>
 *   <li><b>Type Attribute Check:</b> Extract type from expression node semantic attributes
 *       (most reliable for function parameters)</li>
 *   <li><b>Symbol Table Check:</b> Fallback to symbol table lookup to determine if ArrayType/PointerType</li>
 *   <li><b>Conservative Default:</b> If type cannot be determined, return false (no LOAD)</li>
 * </ol>
 * 
 * <p><b>Key Design Decision:</b>
 * 
 * <p>This class uses a <b>conservative approach</b>: if the type cannot be determined,
 * it returns false (no LOAD). This is safe because:
 * <ul>
 *   <li>Struct parameters don't need LOAD (correct behavior)</li>
 *   <li>Scalar parameters don't need LOAD (correct behavior)</li>
 *   <li>Array/pointer parameters that can't be identified will fail at runtime
 *       (better than incorrect code generation)</li>
 * </ul>
 * 
 * <p><b>Example:</b>
 * <pre>
...
public final class ParameterTypeChecker {
     ...
     * 
     * <p>In C, array parameters (int a[]) and pointer parameters (int *a) decay to pointers.
     * The parameter slot contains a pointer value that must be loaded before use.
     * 
     * <p>Struct parameters passed by value (struct Point p) are stored DIRECTLY on the stack
     * and do NOT need dereferencing. This method returns false for struct parameters.
     * 
     * <p><b>Algorithm:</b>
     * <ol>
     *   <li>If parameter size > 4 bytes, it's a struct-by-value → return false (no LOAD needed)</li>
     *   <li>If expressionNode is provided, extract type from semantic attributes (most reliable)</li>
     *   <li>Otherwise, check symbol table to determine if it's ArrayType or PointerType</li>
     *   <li>If type cannot be determined, return false (conservative approach)</li>
     * </ol>
     * 
     * @param variableName the variable name
     * @param expressionNode optional expression node for type extraction (may be null)
     * @return true if the variable is an array or pointer parameter (needs LOAD), 
     *         false if struct-by-value, scalar, or type cannot be determined
     */
    public boolean needsDereferencing(String variableName, NonTerminalNode expressionNode) {
        ...
        // Struct parameters are passed by value and take multiple words (e.g., 8 bytes for struct Point)
     ...
     * 
     * <p>In C, array parameters decay to pointers. This method checks if the variable
     * is a parameter and if its type is an array type (which means it's an array parameter
     * that should be treated as a pointer).
     * 
     * <p><b>Note:</b> Function parameters are stored in function scopes (child scopes of global scope).
     * The standard {@code lookup()} method only traverses parent scopes, so we need to search
     * through child scopes recursively to find function parameters. However, in practice,
     * parameters should be findable via standard lookup if we're in the right context.
     * 
     * @param variableName the variable name
