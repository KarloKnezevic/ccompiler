 ...
 *
 * <p>This class handles the recursive unwrapping of expression nonterminals that don't change the
 * L-value nature of an expression. It's used to extract the actual L-value from expressions wrapped
 * in various grammar layers.
 *
 * <p><b>Purpose:</b>
 *
 ...
 *   <li>{@code <log_ili_izraz>}, {@code <log_i_izraz>} - logical expressions
 *   <li>{@code <bin_ili_izraz>}, {@code <bin_xili_izraz>}, {@code <bin_i_izraz>} - bitwise
 *       expressions
 *   <li>{@code <jednakosni_izraz>}, {@code <odnosni_izraz>} - comparison expressions
 *   <li>{@code <aditivni_izraz>}, {@code <multiplikativni_izraz>} - arithmetic expressions
 *   <li>{@code <cast_izraz>}, {@code <unarni_izraz>} - unary expressions
 * </ul>
 *
 * <p>These layers don't change the L-value nature, so we can safely unwrap them to get to the
 * underlying {@code <postfiks_izraz>} or {@code <primarni_izraz>}.
 *
 * <p><b>Algorithm:</b>
 *
 ...
 *
 * <ol>
 *   <li>Check if the node is a single-child nonterminal
 *   <li>If it's an expression layer (doesn't change L-value), recursively unwrap the child
 *   <li>Stop when reaching {@code <postfiks_izraz>} or {@code <primarni_izraz>}
 * </ol>
 *
 * <p><b>Example:</b>
 *
 ...
 *
 * <ul>
 *   <li>{@code o.middle.inner.arr[0]} remains {@code <postfiks_izraz>} with nested structure
 *   <li>{@code p.x} remains {@code <postfiks_izraz>} with field access structure
 * </ul>
 *
 * <p>This is critical because field access and array indexing are preserved correctly for address
 * generation.
 *
...
public final class ExpressionUnwrapper {
   ...
   *
   * <ul>
   *   <li>{@code <izraz_pridruzivanja>} → unwraps to child
   *   <li>{@code <log_ili_izraz>}, {@code <log_i_izraz>}, {@code <bin_ili_izraz>}, etc. → unwraps
   *       to child
   *   <li>{@code <jednakosni_izraz>}, {@code <odnosni_izraz>}, {@code <aditivni_izraz>}, etc. →
   *       unwraps to child
   *   <li>{@code <multiplikativni_izraz>}, {@code <cast_izraz>} → unwraps to child
   *   <li>{@code <unarni_izraz>} → unwraps to child
   * </ul>
   *
   * <p>Stops when it reaches {@code <postfiks_izraz>} or {@code <primarni_izraz>}, which are the
   * actual L-values.
   *
   ...
   *   <li>{@code o.middle.inner.arr[0]} remains {@code <postfiks_izraz>} with nested structure
   *       intact
   *   <li>{@code p.x} remains {@code <postfiks_izraz>} with field access structure intact
   * </ul>
   *
   ...
   *
   * <p>Expression layers are nonterminals that wrap other expressions but don't change the L-value
   * nature. They can be safely unwrapped to get to the underlying L-value.
   *
