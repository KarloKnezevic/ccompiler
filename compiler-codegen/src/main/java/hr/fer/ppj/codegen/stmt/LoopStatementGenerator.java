 ...
 *
 * <p>This class handles the generation of iterative control flow, implementing the <b>loop code
 * generation algorithm</b> that translates C loop constructs into FRISC assembly with proper label
 ...
 *
 * <ol>
 *   <li><b>Label Generation:</b> Generate unique labels for loop start, continue point, and break
 *       point
 *   <li><b>Context Setup:</b> Create a new code generation context with loop labels for
 *       break/continue
 *   <li><b>Condition Check:</b> Evaluate condition at the start of each iteration
 *   <li><b>Body Execution:</b> Generate code for loop body with break/continue support
 *   <li><b>Iteration Control:</b> Jump back to condition check (continue) or exit loop (break)
 * </ol>
 *
 * <p><b>Loop Types Handled:</b>
 *
 * <ul>
 *   <li><b>While Loops:</b> {@code while (condition) statement}
 *   <li><b>For Loops:</b> {@code for (init; condition; increment) statement}
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li><b>Loop Start Label:</b> Marks the beginning of each iteration
 *   <li><b>Condition Evaluation:</b> Evaluate condition, result in R0
 *   <li><b>Exit Check:</b> If condition is false (R0 == 0), jump to break label
 *   <li><b>Body Execution:</b> Generate loop body code (with break/continue context)
 *   <li><b>Continue Label:</b> Marks the continue point (for continue statements)
 *   <li><b>Loop Back:</b> Jump back to loop start label
 *   <li><b>Break Label:</b> Marks the loop exit point (for break statements)
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li><b>Initialization:</b> Execute initialization expression once (before loop)
 *   <li><b>Loop Start Label:</b> Marks the beginning of each iteration
 *   <li><b>Condition Evaluation:</b> Evaluate condition, result in R0
 *   <li><b>Exit Check:</b> If condition is false (R0 == 0), jump to break label
 *   <li><b>Body Execution:</b> Generate loop body code (with break/continue context)
 *   <li><b>Continue Label:</b> Marks the continue point (for continue statements and increment)
 *   <li><b>Increment:</b> Execute increment expression (if present)
 *   <li><b>Loop Back:</b> Jump back to loop start label
 *   <li><b>Break Label:</b> Marks the loop exit point (for break statements)
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>Break:</b> Jumps to the break label, exiting the loop
 *   <li><b>Continue:</b> Jumps to the continue label, skipping to the next iteration
 * </ul>
 *
 * <p>The loop generator creates a new context with these labels, allowing nested loops to have
 * their own break/continue labels.
 *
 ...
 *
 * <ul>
 *   <li>Each loop creates its own set of labels (loop, continue, break)
 *   <li>Each loop creates a new context with its labels
 *   <li>Break/continue statements in inner loops use the inner loop's labels
 *   <li>Break/continue statements in outer loops use the outer loop's labels
 * </ul>
 *
 ...
 *       actual runtime depends on loop body and iteration count
...
public final class LoopStatementGenerator {
  ...
  private void generateWhileLoop(NonTerminalNode node) {
    ...
    CodeGenContext loopContext =
        context.withLoopLabels(labels.breakLabel(), labels.continueLabel());
