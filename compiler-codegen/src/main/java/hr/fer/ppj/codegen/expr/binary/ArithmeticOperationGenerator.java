 ...
 * <ul>
 *   <li><b>Addition (+):</b> Uses native FRISC ADD instruction</li>
 *   <li><b>Subtraction (-):</b> Uses native FRISC SUB instruction</li>
 *   <li><b>Multiplication (*):</b> Calls F_MUL helper function (FRISC has no native multiply)</li>
 *   <li><b>Division (/):</b> Calls F_DIV helper function (FRISC has no native divide)</li>
 *   <li><b>Modulo (%):</b> Implemented using repeated subtraction algorithm</li>
 * </ul>
 * 
 * <p><b>Algorithm: Operand Evaluation Pattern</b>
 * 
 * <p>All binary arithmetic operations follow the same operand evaluation pattern,
 * implemented by {@link OperandEvaluator}:
 * <ol>
 *   <li><b>Evaluate Left Operand:</b> Recursively generate code for the left expression,
 *       leaving the result in R0</li>
 *   <li><b>Save Left Operand:</b> Push R0 to the stack to preserve it</li>
 *   <li><b>Evaluate Right Operand:</b> Recursively generate code for the right expression,
 *       leaving the result in R0 (overwrites the left operand value)</li>
 *   <li><b>Restore Operands:</b> Move R0 (right operand) to R1, then pop the left operand
 *       back to R0</li>
 *   <li><b>Perform Operation:</b> Execute the arithmetic operation with operands in R0 and R1</li>
 * </ol>
 * 
 * <p><b>FRISC Register Usage:</b>
 * <ul>
 *   <li><b>R0:</b> Left operand (after restoration), then result</li>
 *   <li><b>R1:</b> Right operand</li>
 *   <li><b>R2, R3:</b> Used temporarily in modulo operation</li>
 *   <li><b>Stack:</b> Used to save left operand during right operand evaluation</li>
 * </ul>
 * 
 * <p><b>Native vs Helper Function Operations:</b>
 * 
 * <p>FRISC has native instructions for addition and subtraction, but not for
 * multiplication or division. This class uses different strategies:
 * <ul>
 *   <li><b>Native Operations (+, -):</b> Direct FRISC instructions (ADD, SUB)</li>
 *   <li><b>Helper Functions (*, /):</b> Call software-implemented helper functions
 *       (F_MUL, F_DIV) that perform the operation using basic instructions</li>
 *   <li><b>Algorithm Implementation (%):</b> Modulo is implemented inline using
 *       repeated subtraction (no helper function needed)</li>
 * </ul>
 * 
 ...
 * <ol>
 *   <li><b>Check for Division by Zero:</b> If divisor (b) is 0, result is 0</li>
 *   <li><b>Initialize:</b> Save dividend in R2, divisor in R3</li>
 *   <li><b>Subtraction Loop:</b> Repeatedly subtract divisor from remainder until
 *       remainder < divisor</li>
 *   <li><b>Result:</b> The final remainder (in R2) is the modulo result</li>
 * </ol>
 * 
 * <p>This algorithm is used instead of calling a helper function because:
 * <ul>
 *   <li>Modulo is less common than multiplication/division</li>
 *   <li>The algorithm is simple enough to inline (few instructions)</li>
 *   <li>Avoids the overhead of function call/return</li>
 * </ul>
 * 
 ...
 * <ol>
 *   <li><b>Mark Helper as Needed:</b> Call {@code emitter.markMulNeeded()} or
 *       {@code emitter.markDivNeeded()} to ensure the helper is generated</li>
 *   <li><b>Push Arguments:</b> Push arguments on stack in right-to-left order
 *       (C convention): right operand first, then left operand</li>
 *   <li><b>Call Helper:</b> Call the helper function (F_MUL or F_DIV)</li>
 *   <li><b>Clean Up Stack:</b> Add 8 to R7 to remove the two 4-byte arguments</li>
 *   <li><b>Get Result:</b> Move return value from R6 to R0</li>
 * </ol>
 * 
 ...
 * <p><b>Complexity:</b>
 * <ul>
 *   <li><b>Addition/Subtraction:</b> O(1) code generation, O(1) runtime</li>
 *   <li><b>Multiplication/Division:</b> O(1) code generation, O(n) runtime where n is
 *       the number of bits (helper functions use iterative algorithms)</li>
 *   <li><b>Modulo:</b> O(1) code generation, O(a/b) runtime in worst case (repeated subtraction)</li>
 * </ul>
 * 
...
public final class ArithmeticOperationGenerator {
     ...
     */
    public ArithmeticOperationGenerator(CodeGenContext context, ExpressionCodeGenerator expressionGenerator) {
     ...
     * 
     * <p>This method implements the <b>repeated subtraction algorithm</b> for computing
     * modulo (a % b). The algorithm works by repeatedly subtracting the divisor from
     * the dividend until the remainder is less than the divisor.
     * 
     * <p><b>Algorithm Steps:</b>
     * <ol>
     *   <li><b>Division by Zero Check:</b> If the divisor is 0, the result is 0
     *       (C standard behavior, though undefined in some contexts)</li>
     *   <li><b>Initialize Registers:</b> Save dividend in R2, divisor in R3</li>
     *   <li><b>Subtraction Loop:</b> While remainder (R2) >= divisor (R3):
     *       <ul>
     *         <li>Subtract divisor from remainder: R2 = R2 - R3</li>
     *         <li>Continue loop</li>
     *       </ul>
     *   </li>
     *   <li><b>Result:</b> The final value in R2 is the modulo result</li>
     * </ol>
     * 
     * <p><b>Why Repeated Subtraction Instead of Division?</b>
     * <ul>
     *   <li>FRISC has no native division instruction</li>
     *   <li>Modulo can be computed without the quotient, so we only need the remainder</li>
     *   <li>For small divisors, repeated subtraction is efficient</li>
     *   <li>Avoids the overhead of calling a helper function</li>
     * </ul>
     * 
     * <p><b>Edge Cases:</b>
     * <ul>
     *   <li><b>Division by Zero:</b> If divisor is 0, result is 0 (handled by early exit)</li>
     *   <li><b>Negative Operands:</b> This implementation assumes non-negative operands.
     *       Negative modulo semantics are implementation-defined in C.</li>
     *   <li><b>Large Dividends:</b> The loop may iterate many times for large dividends,
     *       but this is acceptable for the target architecture</li>
     * </ul>
     * 
     * <p><b>FRISC Register Usage:</b>
     * <ul>
     *   <li><b>R0:</b> Initially left operand (dividend), finally result (remainder)</li>
     *   <li><b>R1:</b> Right operand (divisor)</li>
     *   <li><b>R2:</b> Working remainder (starts as dividend, decreases during loop)</li>
     *   <li><b>R3:</b> Saved divisor (constant during loop)</li>
     * </ul>
     * 
