 ...
 * 
 * <p>This class coordinates extraction of function information by delegating
 * to specialized extractors:
 * <ul>
 *   <li>{@link FunctionNameExtractor} - extracts function names</li>
 *   <li>{@link ParameterExtractor} - extracts parameter lists and names</li>
 *   <li>{@link LocalVariableExtractor} - extracts local variable information</li>
 * </ul>
 * 
...
public final class FunctionInfoExtractor {
    ...
    public List<VariableInfo> extractLocalVariables(NonTerminalNode body, int elementSize) {
        ...
        List<LocalVariableExtractor.VariableInfo> extracted = variableExtractor.extractLocalVariables(body, elementSize);
        // Convert to FunctionInfoExtractor.VariableInfo (they have the same structure)
        return extracted.stream()
            .map(v -> new VariableInfo(v.name(), v.sizeInBytes()))
            .toList();
    }
