...
/**
 * Generates FRISC assembly code for primary expressions (literals, identifiers, parenthesized
 * expressions).
 *
 * <p>This class handles the generation of code for the most basic expressions in the C language,
 * which form the foundation of all more complex expressions. It implements the <b>primary
 * expression code generation algorithm</b> that translates C literals and identifiers into FRISC
 * assembly.
 *
 ...
 *
 * <ol>
 *   <li><b>Terminal Identification:</b> Identify the type of primary expression (constant,
 *       identifier, etc.)
 *   <li><b>Type-Aware Code Generation:</b> Generate appropriate FRISC code based on the expression
 *       type:
 *       <ul>
 *         <li><b>Integer Constants:</b> Load using constant loading algorithm (handles 20-bit
 *             immediate limitation)
 *         <li><b>Float Constants:</b> Convert to Q16.16 format, then load as integer
 *         <li><b>Character Constants:</b> Extract ASCII value, load as integer
 *         <li><b>Identifiers:</b> Resolve address (local/global), load value from memory
 *         <li><b>String Literals:</b> Generate label, return address
 *         <li><b>Parenthesized Expressions:</b> Recursively evaluate inner expression (no extra
 *             code)
 *       </ul>
 * </ol>
 *
 ...
 *
 * <p>Integer constants use the constant loading algorithm (see {@link
 * hr.fer.ppj.codegen.emitter.ConstantLoader}):
 *
 * <ul>
 *   <li><b>Small Constants:</b> If value fits in 20-bit signed immediate ([-524288, 524287]), emit
 *       single {@code MOVE %D value, R0} instruction
 *   <li><b>Large Constants:</b> If value exceeds 20-bit range, construct from high and low 16-bit
 *       parts:
 *       <pre>
 ...
 *
 * <ol>
 *   <li><b>Detection:</b> Check if literal contains '.' or 'e'/'E' (exponent notation)
 *   <li><b>Parsing:</b> Parse as float using {@code Float.parseFloat()}
 *   <li><b>Conversion:</b> Convert to Q16.16 format (multiply by 65536, round to integer)
 *   <li><b>Loading:</b> Load Q16.16 value as integer using constant loading algorithm
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li><b>Scope Resolution:</b> Check if variable is local (in activation record) or global
 *   <li><b>Address Resolution:</b> Get FRISC address expression:
 *       <ul>
 *         <li>Local: {@code (R5±offset)} where offset is from activation record
 *         <li>Global: {@code (G_LABEL)} where G_LABEL is the global variable label
 *       </ul>
 *   <li><b>Memory Load:</b> Emit {@code LOAD R0, (address)} instruction
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li><b>Label Generation:</b> Generate unique label (e.g., {@code STR_1})
 *   <li><b>Address Return:</b> Load label address into R0 (string address, not value)
 *   <li><b>Data Generation:</b> String data should be generated in data section (not handled here)
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li>No extra code is generated (parentheses are compile-time only)
 *   <li>Simply delegate to expression generator for inner expression
 *   <li>Result is left in R0 (same as without parentheses)
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Float vs. Integer Detection:</b> Checks both literal format and semantic type
 *   <li><b>Large Constants:</b> Handles constants beyond 20-bit immediate range
 *   <li><b>Invalid Literals:</b> Falls back to zero if parsing fails (never emits invalid code)
...
public final class PrimaryExpressionGenerator {
   ...
   */
  public PrimaryExpressionGenerator(
   ...
   *
   * <ul>
   *   <li>Terminal nodes (BROJ, ZNAK, IDN, NIZ_ZNAKOVA)
   *   <li>Parenthesized expressions: L_ZAGRADA <izraz> D_ZAGRADA
   * </ul>
   *
   * <p><b>FRISC Code Generation:</b>
   *
   * <ul>
   *   <li>Terminals: Generate appropriate load instruction (MOVE for constants, LOAD for variables)
   *   <li>Parentheses: Recursively evaluate inner expression (no extra code)
   * </ul>
   *
  ...
  public void generatePrimaryExpression(NonTerminalNode node) {
    ...
    if (children.size() == 1) {
      ...
      if (first instanceof TerminalNode leftParen
   ...
   *
   * <ul>
   *   <li>{@code BROJ} - Integer or float constant
   *   <li>{@code ZNAK} - Character constant
   *   <li>{@code IDN} - Identifier (variable)
   *   <li>{@code NIZ_ZNAKOVA} - String literal
   * </ul>
   *
   * <p><b>FRISC Code:</b>
   *
   * <ul>
   *   <li>BROJ (int): {@code MOVE %D value, R0} (or SHL/ADD sequence for large values)
   *   <li>BROJ (float): Convert to Q16.16, load into R0
   *   <li>ZNAK: {@code MOVE %D ascii, R0}
   *   <li>IDN: {@code LOAD R0, (address)} where address is (R5±offset) or (G_LABEL)
   *   <li>NIZ_ZNAKOVA: {@code MOVE STR_label, R0} (returns address)
   * </ul>
   *
      ...
      case "BROJ" -> {
        ...
        if (isFloatLiteral || type == PrimitiveType.FLOAT) {
          ...
          try {
            ...
            context
                .emitter()
                .emitLoadIntConstant(q16_16, "R0", "load float constant " + value + " (Q16.16)");
            ...
            try {
              ...
              context
                  .emitter()
                  .emitComment("ERROR: Could not parse float literal: " + value + " - using zero");
            ...
            if (FloatCodegenHelper.isFloatLiteral(value)) {
              ...
              try {
                ...
                context
                    .emitter()
                    .emitLoadIntConstant(
                        q16_16, "R0", "load float constant " + value + " (Q16.16)");
              } catch (NumberFormatException e2) {
                context
                    .emitter()
                    .emitComment(
                        "ERROR: Could not parse float literal: " + value + " - using zero");
                context.emitter().emitLoadIntConstant(0, "R0", "fallback to zero");
              }
            } else {
              ...
              context
                  .emitter()
                  .emitComment("ERROR: Invalid number literal: " + value + " - using zero");
              context.emitter().emitLoadIntConstant(0, "R0", "fallback to zero");
            }
          }
        }
