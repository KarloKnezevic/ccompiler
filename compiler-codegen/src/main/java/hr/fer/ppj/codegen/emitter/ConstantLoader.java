 ...
 * 
 * <p>This class handles the complexity of loading 32-bit integer constants,
 * which may or may not fit in a 20-bit signed immediate. It implements the
 * <b>constant loading algorithm</b> that optimizes code generation based on
 * the constant value.
 * 
 ...
 *       <ul>
 *         <li>If the constant fits in a 20-bit signed immediate (-524288 to 524287),
 *             use a single MOVE instruction</li>
 *         <li>This is the most efficient case: one instruction, one cycle</li>
 *       </ul>
 *   </li>
 *   <li><b>Large Constants (32-bit values):</b>
 *       <ul>
 *         <li>If the constant doesn't fit in 20 bits, construct it from high and
 *             low 16-bit parts</li>
 *         <li>Load the high 16 bits, shift left by 16, then add the low 16 bits</li>
 *         <li>This requires 2-3 instructions depending on whether the low part is zero</li>
 *       </ul>
 *   </li>
 * </ol>
 * 
 ...
 * 
 * <p>FRISC instructions use a 20-bit signed immediate field for constant operands.
 * This means constants in the range [-524288, 524287] can be encoded directly
 * in the instruction, while larger values require construction from multiple parts.
 * 
 ...
 *       <ul>
 *         <li>High part: bits 31-16 (value >> 16)</li>
 *         <li>Low part: bits 15-0 (value & 0xFFFF)</li>
 *       </ul>
 *   </li>
 *   <li><b>Sign Extension:</b> Sign-extend the high part as a signed 16-bit value
 *       to handle negative constants correctly</li>
 *   <li><b>Load High Part:</b> Load the high part into the target register</li>
 *   <li><b>Shift Left:</b> Shift the register left by 16 bits to make room for
 *       the low part</li>
 *   <li><b>Add Low Part:</b> Add the low part to complete the constant (skipped
 *       if low part is zero for optimization)</li>
 * </ol>
 * 
 ...
 * 
 * <p>If the low 16 bits are zero, the ADD instruction is skipped. This optimization
 * reduces code size and execution time for constants that are multiples of 65536.
 * 
 * <p><b>Complexity:</b>
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) - constant number of instructions (1-3)</li>
 *   <li><b>Space Complexity:</b> O(1) - no additional memory required</li>
 * </ul>
 * 
 * <p><b>Register Usage:</b>
 * <ul>
 *   <li><b>Target Register:</b> The register specified by {@code targetRegister}
 *       (typically R0, R6, etc.)</li>
 *   <li><b>No Other Registers:</b> The algorithm only uses the target register,
 *       making it register-efficient</li>
 * </ul>
 * 
...
public final class ConstantLoader {
     ...
     * 
     * <p>This method implements the constant loading algorithm, choosing between
     * a single MOVE instruction (for small constants) or a multi-instruction
     * sequence (for large constants).
     * 
     * <p><b>Algorithm:</b>
     * <ol>
     *   <li><b>Check Immediate Range:</b> If value fits in 20-bit signed immediate
     *       (-524288 to 524287), use single MOVE instruction</li>
     *   <li><b>Split Constant:</b> Otherwise, split into high and low 16-bit parts</li>
     *   <li><b>Sign Extension:</b> Sign-extend high part as signed 16-bit value</li>
     *   <li><b>Load and Shift:</b> Load high part, shift left by 16 bits</li>
     *   <li><b>Add Low Part:</b> Add low part (skipped if zero for optimization)</li>
     * </ol>
     * 
     * <p><b>Preconditions:</b>
     * <ul>
     *   <li>{@code emitter} must not be null</li>
     *   <li>{@code targetRegister} must not be null</li>
     *   <li>{@code value} is a valid 32-bit signed integer</li>
     * </ul>
     * 
     * <p><b>Postconditions:</b>
     * <ul>
     *   <li>The target register contains the constant value</li>
     *   <li>1-3 instructions have been emitted (depending on constant size)</li>
     * </ul>
     * 
     * <p><b>Side Effects:</b>
     * <ul>
     *   <li>Emits FRISC instructions to the emitter buffer</li>
     *   <li>Modifies the target register</li>
     * </ul>
     * 
     ...
     */
    public void emitLoadIntConstant(FriscEmitter emitter, int value, String targetRegister, String comment) {
        ...
        if (value >= IMMEDIATE_MIN && value <= IMMEDIATE_MAX) {
            ...
            emitter.emitInstruction("SHL", targetRegister, "%D 16", targetRegister, "shift hi part left by 16");
