 ...
 * <ul>
 *   <li><b>Primitive types:</b> char (1), int (4), float (4)</li>
 *   <li><b>Array types:</b> element_size × array_length</li>
 *   <li><b>Struct types:</b> Calculated using {@link StructSizeCalculator}</li>
 *   <li><b>Pointer types:</b> 4 bytes</li>
 * </ul>
 * 
 * <p><b>Purpose:</b>
 * 
 * <p>When processing local variable declarations, we need to know the size of each
 * variable to allocate the correct amount of stack space. This class provides a
 * centralized way to calculate variable sizes, handling the complexity of:
 * <ul>
 *   <li>Structs with array fields (requires array size extraction)</li>
 *   <li>Nested structs with arrays (requires recursive extraction)</li>
 *   <li>Arrays of structs (requires struct size calculation)</li>
 * </ul>
 * 
 ...
 * <ol>
 *   <li>Extracts array sizes for struct fields (if the struct has array fields)</li>
 *   <li>Recursively extracts array sizes for nested structs</li>
 *   <li>Calculates the total struct size using {@link StructSizeCalculator}</li>
 * </ol>
 * 
 ...
 * <p>For array types:
 * <ul>
 *   <li>If array size is provided: {@code arraySize × elementSize}</li>
 *   <li>If array size is 0: Uses type system to calculate element size</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li>Arrays: {@code arraySize × elementSize} (default elementSize = 4)</li>
 *   <li>Non-arrays: 4 bytes (default for scalars)</li>
 * </ul>
 * 
...
public final class VariableSizeCalculator {
     ...
     *   <li>Struct types: calculated using {@link StructSizeCalculator}</li>
     ...
     * @throws NullPointerException if arraySize or elementSize are negative (but allows null variableType)
     */
    public int calculateSize(Type variableType, int arraySize, int elementSize) {
        Objects.requireNonNull(arraySize >= 0 ? Integer.valueOf(arraySize) : null, "arraySize must not be negative");
        Objects.requireNonNull(elementSize > 0 ? Integer.valueOf(elementSize) : null, "elementSize must be positive");
        ...
        if (strippedType instanceof StructType structType) {
            ...
            if (strippedElementType instanceof StructType structElementType) {
                ...
                int structSize = StructSizeCalculator.calculateStructSize(structElementType, arraySizes, nestedStructArraySizes);
