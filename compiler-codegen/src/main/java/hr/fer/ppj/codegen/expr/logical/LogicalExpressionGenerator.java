 ...
 * <ul>
 *   <li><b>Logical OR (||):</b> Short-circuits if left operand is true (non-zero)</li>
 *   <li><b>Logical AND (&&):</b> Short-circuits if left operand is false (zero)</li>
 * </ul>
 * 
 * <p><b>Algorithm: Short-Circuit Evaluation</b>
 * 
 * <p>This class implements <b>short-circuit evaluation</b>, a compiler optimization
 * technique where the right operand of a logical expression is only evaluated if
 * the result cannot be determined from the left operand alone. This is both a
 * correctness requirement (C standard) and a performance optimization.
 * 
 * <p><b>Short-Circuit Evaluation Algorithm:</b>
 * 
 * <p>For logical OR (||):
 * <ol>
 *   <li><b>Evaluate Left Operand:</b> Generate code to evaluate the left expression,
 *       leaving the result in R0</li>
 *   <li><b>Check for Short-Circuit:</b> Compare R0 with 0. If R0 != 0 (true),
 *       jump to the "true" label, setting result to 1 and skipping right operand</li>
 *   <li><b>Evaluate Right Operand:</b> If left is false, evaluate the right expression</li>
 *   <li><b>Compute Final Result:</b> If right is true (non-zero), result is 1, else 0</li>
 * </ol>
 * 
 * <p>For logical AND (&&):
 * <ol>
 *   <li><b>Evaluate Left Operand:</b> Generate code to evaluate the left expression,
 *       leaving the result in R0</li>
 *   <li><b>Check for Short-Circuit:</b> Compare R0 with 0. If R0 == 0 (false),
 *       jump to the "false" label, setting result to 0 and skipping right operand</li>
 *   <li><b>Evaluate Right Operand:</b> If left is true, evaluate the right expression</li>
 *   <li><b>Compute Final Result:</b> If right is true (non-zero), result is 1, else 0</li>
 * </ol>
 * 
 ...
 * <ul>
 *   <li><b>Evaluation Block:</b> Evaluates left operand, checks for short-circuit</li>
 *   <li><b>Right Operand Block:</b> Evaluates right operand (only reached if short-circuit
 *       doesn't occur)</li>
 *   <li><b>Result Blocks:</b> Sets final result (true/false) and jumps to end</li>
 * </ul>
 * 
 ...
 * 
 * <ul>
 *   <li><b>Correctness:</b> The C standard requires short-circuit evaluation.
 *       Code like {@code if (ptr != NULL && ptr->value > 0)} must not dereference
 *       a null pointer.</li>
 *   <li><b>Performance:</b> Avoiding unnecessary evaluation of expensive operations
 *       (function calls, complex expressions) improves runtime performance</li>
 *   <li><b>Side Effects:</b> Short-circuit evaluation affects programs that rely on
 *       side effects in the right operand (though such code is generally considered
 *       poor style)</li>
 * </ul>
 * 
 * <p><b>Register Usage:</b>
 * <ul>
 *   <li><b>R0:</b> Holds the result of expression evaluation and the final boolean result (0 or 1)</li>
 *   <li><b>Other Registers:</b> May be used temporarily during operand evaluation</li>
 * </ul>
 * 
 * <p><b>Label Management:</b>
 * 
 * <p>This class uses {@link hr.fer.ppj.codegen.util.LabelGenerator#generateShortCircuitLabels()}
 * to generate unique labels for each logical expression. The labels follow the pattern:
 * <ul>
 *   <li>{@code L_SC_TRUE<n>}: Label for the true result case</li>
 *   <li>{@code L_SC_FALSE<n>}: Label for the false result case (AND only)</li>
 *   <li>{@code L_SC_END<n>}: Label for the end of the evaluation</li>
 * </ul>
 * 
 * <p><b>Complexity:</b>
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) for code generation (constant number of instructions),
 *       but the actual runtime depends on operand evaluation complexity</li>
 *   <li><b>Space Complexity:</b> O(1) for code generation (fixed number of labels and instructions)</li>
 * </ul>
 * 
 * <p>Logical expressions return 1 (true) or 0 (false) in register R0, following
 * the C convention where zero is false and any non-zero value is true.
 * 
...
public final class LogicalExpressionGenerator {
     ...
     *       The result is left in R0.</li>
     *   <li><b>Check for Short-Circuit:</b> Compare R0 with 0. If R0 != 0 (left is true),
     *       jump to the true label. This is the short-circuit: we skip evaluating the
     *       right operand because the result is already known to be true.</li>
     *   <li><b>Evaluate Right Operand:</b> If left is false (R0 == 0), we must evaluate
     *       the right operand to determine the final result. Generate code for the right
     *       expression, leaving the result in R0.</li>
     *   <li><b>Compute Final Result:</b> Compare the right operand result with 0. If it's
     *       non-zero (true), jump to the true label. Otherwise, set result to 0 (false)
     *       and jump to the end label.</li>
     *   <li><b>True Case:</b> At the true label, set R0 to 1 (true). This label is reached
     *       either from the short-circuit (left is true) or from the right operand check
     *       (right is true).</li>
     * </ol>
     * 
     ...
     * <ul>
     *   <li>Uses conditional jumps (JP_NE, JP) to implement control flow</li>
     *   <li>Result is always 0 (false) or 1 (true) in R0</li>
     *   <li>Right operand is only evaluated if left operand is false</li>
     * </ul>
     * 
     * <p><b>Side Effects:</b>
     * <ul>
     *   <li>Emits FRISC instructions to the emitter buffer</li>
     *   <li>Generates unique labels for control flow</li>
     *   <li>Leaves the final result (0 or 1) in register R0</li>
     * </ul>
     * 
     ...
     *   <li><b>Evaluate Left Operand:</b> Recursively generate code for the left expression.
     *       The result is left in R0.</li>
     *   <li><b>Check for Short-Circuit:</b> Compare R0 with 0. If R0 == 0 (left is false),
     *       jump to the false label. This is the short-circuit: we skip evaluating the
     *       right operand because the result is already known to be false (false && anything = false).</li>
     *   <li><b>Evaluate Right Operand:</b> If left is true (R0 != 0), we must evaluate
     *       the right operand to determine the final result. Generate code for the right
     *       expression, leaving the result in R0.</li>
     *   <li><b>Compute Final Result:</b> Compare the right operand result with 0. If it's
     *       non-zero (true), jump to the true label. Otherwise, set result to 0 (false)
     *       and jump to the end label.</li>
     *   <li><b>True Case:</b> At the true label, set R0 to 1 (true). This is only reached
     *       if both operands are true.</li>
     *   <li><b>False Case:</b> At the false label, set R0 to 0 (false). This is reached
     *       either from the short-circuit (left is false) or from the right operand check
     *       (right is false).</li>
     * </ol>
     * 
     ...
     * <p><b>FRISC Semantics:</b>
     * <ul>
     *   <li>Uses conditional jumps (JP_EQ, JP_NE, JP) to implement control flow</li>
     *   <li>Result is always 0 (false) or 1 (true) in R0</li>
     *   <li>Right operand is only evaluated if left operand is true</li>
     * </ul>
     * 
     * <p><b>Key Difference from Logical OR:</b>
     * <ul>
     *   <li>Logical AND short-circuits when the <i>left</i> operand is <i>false</i></li>
     *   <li>Logical OR short-circuits when the <i>left</i> operand is <i>true</i></li>
     *   <li>This asymmetry requires different jump conditions (JP_EQ vs JP_NE)</li>
     * </ul>
     * 
     * <p><b>Side Effects:</b>
     * <ul>
