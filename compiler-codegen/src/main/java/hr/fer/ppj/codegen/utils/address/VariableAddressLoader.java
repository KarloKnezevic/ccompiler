 ...
 * 
 * <p>This class handles the low-level code generation for loading variable addresses,
 * including handling different address formats (global labels vs. frame pointer offsets)
 * and parameter dereferencing logic.
 * 
 * <p><b>Purpose:</b>
 * 
 * <p>When computing addresses for L-values, we need to generate FRISC instructions that:
 * <ul>
 *   <li>Load global variable labels (e.g., {@code MOVE G_X, R0})</li>
 *   <li>Compute frame pointer offsets for local variables (e.g., {@code MOVE R5, R0; ADD R0, -08, R0})</li>
 *   <li>Handle parameter dereferencing for array/pointer parameters (e.g., {@code LOAD R0, (R0)})</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li><b>Global Variables:</b> {@code (G_LABEL)} → generates {@code MOVE G_LABEL, targetRegister}</li>
 *   <li><b>Local Variables/Parameters:</b> {@code (R5+offset)} or {@code (R5-offset)} → generates
 *       frame pointer computation and optional offset addition</li>
 * </ul>
 * 
 * <p><b>Parameter Dereferencing:</b>
 * 
 * <p>For array/pointer parameters, the parameter slot contains a pointer value that must
 * be loaded. This class uses {@link ParameterTypeChecker} to determine if dereferencing
 * is needed and emits the appropriate LOAD instruction.
 * 
...
public final class VariableAddressLoader {
     ...
     * 
     * <p><b>Array Parameters:</b> In C, array parameters decay to pointers. When the variable
     * is an array parameter (declared as {@code int a[]} or {@code int *a}), the parameter slot
     * contains a pointer value (the base address of the array). To get the base address for
     * array indexing, we need to LOAD the pointer value from the parameter slot.
     * 
     * <p><b>Struct Parameters:</b> Struct parameters passed by value are stored DIRECTLY on the stack.
     * They do NOT decay to pointers like arrays do. For struct parameters, we should NOT emit the extra LOAD.
     * 
     ...
     * @param expressionNode optional expression node for type extraction (if null, falls back to symbol table lookup)
     */
    public void loadAddress(String address, String targetRegister, String variableName, NonTerminalNode expressionNode) {
     ...
     * <ul>
     *   <li>Loads R5 into target register</li>
     *   <li>Adds/subtracts the offset to get the final address</li>
     *   <li>For array/pointer parameters, emits LOAD to dereference the pointer</li>
     * </ul>
     * 
     ...
     */
    private void loadFramePointerAddress(String address, String targetRegister, String variableName, NonTerminalNode expressionNode) {
        // Load frame pointer
        ...
        if (offsetStr != null) {
            ...
            if (offsetStr.startsWith("+")) {
                ...
                if (typeChecker.needsDereferencing(variableName, expressionNode)) {
                    ...
                    context.emitter().emitInstruction("LOAD", targetRegister, "(" + targetRegister + ")", 
