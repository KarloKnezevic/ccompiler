 ...
 *
 * <p>This class handles the generation of code for struct member access (field access) expressions
 * like {@code struct.field}, implementing the <b>field access code generation algorithm</b> that
 * translates C struct field operations into FRISC assembly with proper address calculation.
 *
 ...
 *       <ul>
 *         <li>Local struct: Use frame pointer with offset (e.g., {@code R5 - 20})
 *         <li>Global struct: Use global label (e.g., {@code G_P})
 *         <li>Nested field access: Recursively compute base address
 *         <li>Array element: Compute array element address first
 *       </ul>
 *   <li><b>Field Offset Lookup:</b> Get the byte offset of the field within the struct
 *   <li><b>Field Address Calculation:</b> Add field offset to base address: {@code ADD R0, %D
 *       offset, R0}
 *   <li><b>Memory Access:</b> Load or store field value using computed address:
 *       <ul>
 *         <li>int/float/pointer fields: Use {@code LOAD} / {@code STORE}
 *         <li>char fields: Use {@code LOADB} / {@code STOREB}
 *       </ul>
 * </ol>
 *
 * <p><b>Field Offset Calculation:</b>
 *
 * <p>Field offsets are calculated using {@link
 * hr.fer.ppj.codegen.structs.StructFieldOffsetCalculator}:
 *
 * <ul>
 *   <li>Fields are laid out in declaration order
 *   <li>No padding between fields (tightly packed)
 *   <li>Offset = sum of sizes of all preceding fields
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li>Compute address of {@code o} (struct Outer object)
 *   <li>Add offset of {@code inner} field → address of inner struct
 *   <li>Add offset of {@code value} field → final field address
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li>Compute address of {@code o}
 *   <li>Add offset of {@code arr} field → base address of array
 *   <li>Calculate array element address: base + index × element_size
 * </ol>
 *
...
public final class FieldAccessGenerator {
  ...
  public void generateFieldAccess(NonTerminalNode base, String fieldName) {
    ...
    Integer fieldOffset =
        StructFieldOffsetCalculator.getFieldOffset(
            structType, fieldName, arraySizes, nestedStructArraySizes);
    if (fieldOffset == null) {
   ...
   *
   * <p>This method computes the address of a field without loading its value, which is needed for
   * assignments like {@code p.x = value}.
   *
   * <p>For nested field access like {@code o.inner.value}, this method:
   *
   * <ol>
   *   <li>Uses the address generator to recursively compute the base address
   *   <li>Adds the field offset to get the final field address
   * </ol>
   *
   * @param base the base expression (struct object)
  ...
  public void generateFieldStore(NonTerminalNode base, String fieldName, String sourceRegister) {
    ...
    context
        .emitter()
        .emitInstruction("STORE", tempRegister, "(R0)", "store field '" + fieldName + "'");
  }
}
