...
/**
 * Lexical analyzer (lexer) that tokenizes input text using generated
 * deterministic finite automata (DFAs).
 * 
 * <p>
 * This lexer implements the following algorithms and mechanisms:
 * 
 * <ul>
 * <li><strong>Algorithm B - Maximal Munch (P2):</strong> Always selects the
 * longest possible match.
 * If multiple rules match the same length, rule order (P3) is used to break
 * ties.</li>
 * <li><strong>Algorithm B - Rule Order (P3):</strong> Earlier rules in the
 * specification have
 * priority over later rules that match the same length.</li>
 * <li><strong>Algorithm C - Error Recovery:</strong> If an unrecognized
 * character is encountered,
 * the first character is discarded and analysis continues (panic mode
 * recovery).</li>
 * </ul>
 * 
 * <p>
 ...
 * <ul>
 * <li><strong>UDJI_U_STANJE &lt;state&gt;</strong>: Changes the current lexer
 * state.
 * Used for comments, string literals, etc. This allows the lexer to switch
 * between
 * different sets of matching rules based on context.</li>
 * <li><strong>VRATI_SE &lt;n&gt;</strong> (equivalent to yyless(n)): Returns
 * characters to the input stream.
 * Of the matched characters, the first n characters are grouped into the
 * lexical token,
 * while the remaining characters are returned to the input buffer for the next
 * tokenization step.</li>
 * <li><strong>NOVI_REDAK</strong>: Increments the line number for the next
 * lexical token.
 * This is used when a newline character is encountered or when explicitly
 * triggered by an action.</li>
 * </ul>
 * 
 * <p>
 * <strong>Symbol Table:</strong>
 * Every token is automatically added to the symbol table. Tokens with the same
 * type and text
 * share the same index in the symbol table, enabling efficient storage and
 * lookup.
 * 
 * <p>
 ...
 * 
 * <p>
 * <strong>Lexer States:</strong>
 * The lexer maintains a current state (e.g., S_pocetno, S_string, S_komentar)
 * that determines
 * which set of DFA rules are active. State transitions occur when UDJI_U_STANJE
 ...
 * <li>Unterminated string literals are detected and reported with proper error
 * messages</li>
 * <li>Error recovery ensures the lexer can continue processing after
 * encountering errors</li>
 * </ul>
 * 
...
public final class Lexer {
   ...
   * Each state has its own DFA that defines which patterns can be matched in that
   * state.
   */
   ...
   * Tokens with the same type and text share the same index, enabling efficient
   * storage.
   */
   ...
   * 
   * <p>
   * This record represents a single entry in the symbol table. Multiple tokens
   ...
   * 
   * <p>
   * The generator result contains all the DFAs for each lexer state, which are
   * used during tokenization to match patterns and generate tokens.
   * 
   * @param generatorResult Result of lexer generation containing DFAs for each
   *                        state
   */
   ...
   * 
   * <p>
   * <strong>Tokenization Algorithm:</strong>
   * <ol>
   * <li>Read input text line by line and add it to the buffer</li>
   * <li>For each line, attempt to find the longest match using the DFA for the
   * current state</li>
   * <li>If a match is found, create a token and add it to the list (unless it's
   * whitespace/comment)</li>
   * <li>If no match is found, check if the state changed (VRATI_SE 0 with
   * UDJI_U_STANJE)</li>
   * <li>If nothing happened, apply Algorithm C - discard the first character and
   * print an error</li>
   * </ol>
   * 
   * <p>
   * <strong>Maximal Munch (P2):</strong>
   ...
   * 
   * <p>
   * <strong>Error Recovery (Algorithm C):</strong>
   * If an unrecognized character is encountered (a character for which there is
   * no transition in the DFA),
   * the first character is discarded from the buffer, an error is printed to
   * stderr, and analysis continues.
   * 
   * <p>
   * <strong>State Management:</strong>
   * The lexer maintains a current state that determines which DFA is used for
   * matching.
   ...
   * <ul>
   * <li>String literals: Stops matching at the closing quote to prevent
   * over-matching</li>
   * <li>Unterminated strings: Detects and reports errors, then recovers by
   * exiting the string state</li>
  ...
  /**
   * Tokenizes the input text and returns a list of tokens.
   * Reports errors to the provided DiagnosticReporter.
   * 
   * @param input    Reader that reads the input text
   * @param reporter DiagnosticReporter to collecting errors
   * @return List of tokens found in the input text
   * @throws IOException if an error occurs while reading the input
   */
  public List<Token> tokenize(Reader input, hr.fer.ppj.common.diagnostic.DiagnosticReporter reporter)
      throws IOException {
    ...
    // Continue processing buffer even when there are no more lines (to process
    // remaining content)
    ...
    while (reading || buffer.length() > 0) {
      ...
      while (buffer.length() > 0 && iterations < maxIterations) {
        ...
        if (token != null) {
          ...
          // Promjena stanja znači da treba nastaviti
          ...
          if (state.getCurrentState().equals("S_string") && buffer.length() > 0) {
            ...
            if (firstChar == '\n') {
              int stringStart = findStringStart(buffer);

              int errorLine = state.getLineNumber();
              int errorCol = Math.max(1, state.getColumnNumber() - buffer.length() + stringStart);

              reportError(reporter, errorLine, errorCol, "nezatvoren string literal");
          ...
          if (buffer.length() > 0) {
            ...
            reportError(reporter, errorLine, errorCol, String.format("neprepoznat znak '%c' (0x%02x)", c, (int) c));
      ...
      if (iterations >= maxIterations) {
        String msg = "Lexer loop exceeded maximum iterations, possible infinite loop";
        if (reporter != null) {
          reporter.error(hr.fer.ppj.common.diagnostic.Stage.LEXER,
              new hr.fer.ppj.common.source.SourceLocation(state.getLineNumber(), state.getColumnNumber()),
              msg);
        } else {
  ...
  public List<Token> tokenize(Reader input) throws IOException {
    return tokenize(input, new hr.fer.ppj.common.diagnostic.DiagnosticReporter() {
      @Override
      public void report(hr.fer.ppj.common.diagnostic.Diagnostic diagnostic) {
        System.err.println("Leksička greška na retku " + diagnostic.location().line() + ", stupcu "
  ...
  private int findStringStart(StringBuilder buffer) {
  ...
  private void reportError(hr.fer.ppj.common.diagnostic.DiagnosticReporter reporter, int line, int col, String msg) {
    if (reporter != null) {
      reporter.report(hr.fer.ppj.common.diagnostic.Diagnostic.error(
          hr.fer.ppj.common.diagnostic.Stage.LEXER,
          new hr.fer.ppj.common.source.SourceLocation(line, col),
          msg));
    }
  }
   ...
   * 
   * <p>
   * This method implements the maximal munch algorithm (P2) to find the longest
   * possible match. It traverses the DFA character by character, tracking the
   * longest
   * accepting state encountered. When no more transitions are available, it
   * returns
   ...
   * <li>For each character in the input buffer, follow transitions in the
   * DFA</li>
   ...
   * <li>Handle special cases (e.g., string literals stopping at closing
   * quote)</li>
   * </ol>
   * 
   * <p>
   ...
   * <li>String literals: Stops matching at the closing quote to prevent
   * over-matching</li>
   * <li>Escaped quotes: Continues matching if the quote is escaped (part of
   * string content)</li>
   * </ul>
   * 
  ...
  private Token scanToken(StringBuilder input, LexerState lexerState) {
    ...
    for (int i = 0; i < input.length(); i++) {
      ...
      if (dfa.isAccepting(currentState)) {
        ...
        // BUT: We need to check if this is an escaped quote (\") or a real closing
        // quote.
        // If the previous character was a backslash, this is an escaped quote, not a
        // closing quote.
    ...
    //
    // According to specification: "of the read characters, group the first n
    // characters
    // into the lexical token, and return the remaining characters to the input
    // stream"
    //
    // Examples:
    // - VRATI_SE 0: group 0 characters, return all (character " remains in buffer
    // for next match)
    ...
    // Characters from actualMatch to longestMatch remain in the buffer (they're
    // already there)
    input.delete(0, actualMatch);

    // Save line/column number BEFORE processing the match
    // According to specification: "The incremented line number applies only to the
    // next lexical token"
    ...
    // Update line/column number for the matched portion (only for actually consumed
    // characters)
    ...
    if (longestActions != null) {
      for (String action : longestActions) {
        if (action.startsWith("UDJI_U_STANJE")) {
          ...
          // If \n was already found in matchedText, we already incremented in the loop
          // above
   ...
   * 
   * <p>
   * Tablica simbola sadrži sve jedinstvene kombinacije (tip tokena, tekst
   * tokena).
   * Ako kombinacija već postoji, vraća se postojeći indeks. Inače se dodaje novi
   * unos.
   * 
   * <p>
   * Ovo osigurava da tokeni s istim tipom i tekstom dijele isti indeks u tablici
   * simbola,
