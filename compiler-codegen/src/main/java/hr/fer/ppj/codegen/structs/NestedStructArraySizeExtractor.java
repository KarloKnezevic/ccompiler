 ...
 * 
 * <p>This class provides a centralized method for extracting array sizes
 * for all nested structs at any depth within a struct type. It ensures that
 * when processing a struct like {@code Outer} that contains {@code Middle},
 * which contains {@code Inner}, all array sizes are extracted recursively.
 * 
 * <p><b>Algorithm: Nested Struct Array Size Extraction</b>
 * 
 * <p>The algorithm works as follows:
 * <ol>
 *   <li><b>Current Struct Extraction:</b> Extract array sizes for the current struct
 *       (if it has a tag)</li>
 *   <li><b>Field Traversal:</b> For each field that is a struct type, recursively
 *       extract array sizes</li>
 *   <li><b>Depth-First Traversal:</b> Process nested structs before processing
 *       deeper nested structs</li>
 * </ol>
 * 
 * <p><b>Critical Design Decision:</b>
 * 
 * <p>Array sizes are always extracted and stored in the map, even if empty.
 * This is important because:
 * <ul>
 *   <li>An empty map indicates that we've tried to extract array sizes for this struct</li>
 *   <li>This prevents infinite recursion when the same struct appears multiple times</li>
 *   <li>It allows {@link StructSizeCalculator} to know that array sizes were attempted</li>
 * </ul>
 * 
 * <p><b>Example:</b>
 * <pre>
 ...
 * <ul>
 *   <li><b>Time Complexity:</b> O(n) where n is the total number of nested struct fields</li>
 *   <li><b>Space Complexity:</b> O(n) for storing array sizes in the map</li>
 * </ul>
 * 
...
public final class NestedStructArraySizeExtractor {
     ...
     * 
     * <p><b>Critical:</b> Always extracts and stores array sizes in the map, even if empty.
     * This ensures that we know we've attempted extraction for each struct, preventing
     * infinite recursion and allowing proper size calculation.
     * 
     ...
     *        (struct tag -> array sizes map)
     */
    public static void extractNestedStructArraySizes(StructType structType, 
