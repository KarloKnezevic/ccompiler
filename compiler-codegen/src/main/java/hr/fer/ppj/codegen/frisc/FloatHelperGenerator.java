 ...
 * 
 * <p>FRISC architecture has no native hardware support for floating-point operations.
 * This class coordinates the generation of software routines that implement floating-point
 * arithmetic using <b>Q16.16 fixed-point representation</b> by delegating to specialized
 * generators. It acts as a <b>facade</b> that manages the generation of all float helper
 * functions based on which operations are actually needed by the generated code.
 * 
 * <p><b>Design Pattern: Facade</b>
 * 
 * <p>This class implements the <b>facade pattern</b>, providing a simplified interface
 * for generating multiple related helper functions. It:
 * <ul>
 ...
 * <ul>
 *   <li><b>Bits 31-16:</b> Integer part (signed 16-bit, range -32768 to 32767)</li>
 *   <li><b>Bits 15-0:</b> Fractional part (unsigned 16-bit, represents value/65536)</li>
 *   <li><b>Scaling Factor:</b> 65536 = 2^16</li>
 *   <li><b>Actual Value:</b> stored_integer / 65536.0</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li><b>Simplicity:</b> Fixed-point arithmetic is simpler to implement in software</li>
 *   <li><b>Performance:</b> Integer operations are faster than floating-point emulation</li>
 *   <li><b>Precision:</b> Q16.16 provides sufficient precision for most applications
 *       (16 bits of fractional precision ≈ 4-5 decimal digits)</li>
 *   <li><b>Range:</b> Supports values from approximately -32768.0 to 32767.99998</li>
 *   <li><b>No Special Cases:</b> No need to handle infinity, NaN, or denormalized numbers</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li><b>{@link FloatAddGenerator}:</b> Generates F_FADD for float addition.
 *       Simple integer addition (O(1)).</li>
 *   <li><b>{@link FloatSubGenerator}:</b> Generates F_FSUB for float subtraction.
 *       Simple integer subtraction (O(1)).</li>
 *   <li><b>{@link FloatMulGenerator}:</b> Generates F_FMUL for float multiplication.
 *       Uses 64-bit multiplication and Q16.16 conversion (O(32)).</li>
 *   <li><b>{@link FloatDivGenerator}:</b> Generates F_FDIV for float division.
 *       Uses 64-bit division (O(64)).</li>
 *   <li><b>{@link FloatCmpGenerator}:</b> Generates F_FCMP for float comparison.
 *       Simple integer comparison (O(1)).</li>
 *   <li><b>{@link IntToFloatGenerator}:</b> Generates F_I2F for integer to float conversion.
 *       Left shift by 16 bits (O(1)).</li>
 *   <li><b>{@link FloatToIntGenerator}:</b> Generates F_F2I for float to integer conversion.
 *       Right shift by 16 bits (O(1)).</li>
 * </ul>
 * 
 ...
 * float helpers, ensuring they are generated later in the code generation pipeline.
 * 
 * <p><b>Lazy Generation Strategy:</b>
 * 
 * <p>Helpers are only generated if they are actually needed:
 * <ol>
 *   <li>During expression code generation, float operations call methods like
 *       {@code emitter.markFloatAddNeeded()} to mark which helpers are needed</li>
 *   <li>After processing the translation unit, the main code generator queries
 *       which float helpers are needed</li>
 ...
 *       (typically 0-7 helpers, each O(1) to O(64) depending on the helper)</li>
