 ...
 * 
 * <p>This class provides methods to compute the memory size of struct types,
 * including handling of nested structs and arrays within structs. It implements
 * the <b>struct size calculation algorithm</b> used throughout code generation.
 * 
 ...
 *       <ul>
 *         <li>Arrays: element_size × array_length (from arraySizes map)</li>
 *         <li>Nested structs: recursively calculate struct size</li>
 *         <li>Other types: delegate to {@link TypeSizeCalculator}</li>
 *       </ul>
 *   </li>
...
public final class StructSizeCalculator {
     ...
     * of all fields in declaration order. It assumes the struct has no array fields.
     * 
     * <p><b>Use Case:</b> Simple structs without arrays (e.g., {@code struct Point { int x; int y; }})
     * 
     * <p><b>Limitation:</b> If the struct contains array fields, this method will throw
     * {@link IllegalArgumentException}. Use the overload that accepts array sizes instead.
     * 
     ...
     * <ul>
     *   <li>Key: Field name (e.g., "data")</li>
     *   <li>Value: Array length (e.g., 10)</li>
     * </ul>
     * 
     * <p><b>Example:</b>
     * <pre>
     ...
     * 
     * <p><b>Critical Design Decision:</b> If a struct tag exists in nestedStructArraySizes,
     * those array sizes are used instead of the arraySizes parameter. This ensures that when
     * calculating size for nested structs (like Inner inside Middle), we use the correct array
     * sizes for the nested struct itself, not the parent struct.
     * 
     * @param structType the struct type to calculate size for
    ...
    public static int calculateStructSize(StructType structType, Map<String, Integer> arraySizes, 
        ...
        for (Map.Entry<String, Type> field : structType.fields().entrySet()) {
            ...
            if (fieldType instanceof ArrayType arrayType) {
                ...
                int elementSize = TypeSizeCalculator.calculateTypeSize(arrayType.elementType(), nestedStructArraySizes);
