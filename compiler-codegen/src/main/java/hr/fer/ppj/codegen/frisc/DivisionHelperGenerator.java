 ...
 * 
 * <p>FRISC architecture does not have a native DIV instruction, so division
 * is implemented using the <b>binary long division algorithm</b> (also known as
 * the <b>shift-subtract algorithm</b> or <b>restoring division</b>).
 * 
 * <p><b>Algorithm: Binary Long Division (Shift-Subtract Algorithm)</b>
 * 
 * <p>This algorithm performs division by processing the dividend bit-by-bit,
 * similar to long division by hand. The algorithm works as follows:
 * 
 * <ol>
 *   <li><b>Sign Handling:</b> Convert both operands to positive, track the sign
 *       of the result using XOR (negative ÷ negative = positive, etc.)</li>
 *   <li><b>Initialization:</b> Set quotient (q) = 0, remainder (r) = 0</li>
 *   <li><b>Binary Loop:</b> For each bit position i from 31 down to 0:
 *       <ul>
 *         <li>Shift remainder left by 1: r <<= 1</li>
 *         <li>Bring down the i-th bit of the dividend into the remainder:
 *             r |= (dividend >> i) & 1</li>
 *         <li>If remainder >= divisor:
 *             <ul>
 *               <li>Subtract divisor from remainder: r -= divisor</li>
 *               <li>Set the i-th bit of the quotient: q |= (1 << i)</li>
 *             </ul>
 *         </li>
 *       </ul>
 *   </li>
 *   <li><b>Sign Application:</b> If the original sign was negative, negate the quotient</li>
 * </ol>
 * 
 * <p><b>Mathematical Basis:</b>
 * 
 * <p>The algorithm is based on the binary representation of numbers and works
 * similarly to long division in base 10. For example, dividing 91 ÷ 7:
 * <pre>
 * 91 = 1011011₂ (binary)
 * 7  = 111₂ (binary)
 * 
 ...
 * <ul>
 *   <li><b>Time Complexity:</b> O(32) for 32-bit integers. The loop iterates
 *       exactly 32 times (once per bit position).</li>
 *   <li><b>Space Complexity:</b> O(1) - uses only a few registers</li>
 *   <li><b>Comparison with Naive Algorithm:</b> The naive algorithm (repeated
 *       subtraction) would be O(dividend/divisor), which can be very large.
 *       This algorithm is much more efficient.</li>
 * </ul>
 * 
 * <p><b>Edge Cases Handled:</b>
 * <ul>
 *   <li><b>Division by Zero:</b> If divisor == 0, return 0 (C standard says
 *       division by zero is undefined behavior, but we handle it gracefully)</li>
 *   <li><b>Negative Operands:</b> Convert to positive, track sign, apply sign
 *       to quotient (C standard: truncation towards zero)</li>
 *   <li><b>Truncation Towards Zero:</b> The algorithm implements C's truncation
 *       semantics: -7/3 = -2, 7/-3 = -2 (not -3)</li>
 * </ul>
 * 
 * <p><b>FRISC Register Usage:</b>
 * <ul>
 *   <li><b>R0:</b> Quotient (q), being built bit-by-bit</li>
 *   <li><b>R1:</b> Divisor (b), constant during the loop</li>
 *   <li><b>R2:</b> Remainder (r), updated each iteration</li>
 *   <li><b>R3:</b> Loop counter (i), counts from 31 down to 0</li>
 *   <li><b>R4:</b> Sign flag (0 = positive, 1 = negative), also used to store
 *       dividend copy during loop</li>
 *   <li><b>R5:</b> Temporary (used for bit extraction and bit setting)</li>
 *   <li><b>R6:</b> Return value (final quotient)</li>
 * </ul>
 * 
 ...
 * <ul>
 *   <li><b>Efficiency:</b> O(log n) instead of O(n) for naive repeated subtraction</li>
 *   <li><b>Correctness:</b> Handles all edge cases (zero, negative, truncation)</li>
 *   <li><b>Simplicity:</b> Easy to implement in assembly with basic shift and subtract</li>
 *   <li><b>No Hardware Dependency:</b> Works on any architecture with shift and subtract</li>
 * </ul>
 * 
