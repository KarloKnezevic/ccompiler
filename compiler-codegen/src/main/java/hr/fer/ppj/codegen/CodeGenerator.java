 ...
 * results (symbol table and annotated parse tree) and producing FRISC assembly code in the file
 * {@code a.frisc}. It implements a <b>syntax-directed translation</b> approach, where the structure
 * of the generated code mirrors the structure of the source parse tree.
 *
 ...
 *       <ul>
 *         <li>Create shared infrastructure ({@link FriscEmitter}, {@link LabelGenerator})
 *         <li>Initialize code generation context ({@link CodeGenContext})
 *         <li>Generate program entry point (stack initialization, main call, halt)
 *       </ul>
 *   <li><b>Translation Unit Processing Phase:</b>
 *       <ul>
 *         <li>Traverse the parse tree to identify all functions and global declarations
 *         <li>Generate code for each function definition ({@link FunctionCodeGenerator})
 *         <li>Track which helper functions are needed (F_MUL, F_DIV, float helpers)
 *       </ul>
 *   <li><b>Helper Function Generation Phase:</b>
 *       <ul>
 *         <li>Generate float helper functions first ({@link FloatHelperGenerator})
 *         <li>Float helpers may internally require integer helpers (F_MUL, F_DIV)
 *         <li>Generate integer helper functions if needed ({@link HelperFunctionGenerator})
 ...
 *             emitter flags are set during float helper generation.
 *       </ul>
 *   <li><b>Global Variable Generation Phase:</b>
 *       <ul>
 *         <li>Generate global variable declarations ({@link GlobalVariableGenerator})
 *         <li>Extract and emit initializer values for initialized globals
 *         <li>Allocate uninitialized arrays with appropriate size directives
 *       </ul>
 *   <li><b>Code Emission Phase:</b>
 *       <ul>
 *         <li>Write all buffered assembly code to the output file
 *         <li>Format instructions, labels, and comments consistently
 *       </ul>
 * </ol>
 *
 * <p><b>Grammar Rule:</b> This class processes the root {@code <prijevodna_jedinica>} (translation
 * unit) nonterminal, which contains all external declarations (function definitions and global
 ...
 *
 * <ul>
 *   <li><b>R7 (Stack Pointer):</b> Initialized to 40000, grows downward (decreasing addresses)
 *   <li><b>R5 (Frame Pointer):</b> Points to the saved old R5 in the current stack frame
 *   <li><b>R6 (Return Value):</b> Used to return function results
 *   <li><b>R0-R4:</b> General-purpose registers (caller-saved, may be clobbered)
 *   <li><b>Parameter Passing:</b> Arguments pushed on stack right-to-left (C convention)
 *   <li><b>Stack Frame Layout:</b>
 *       <ul>
 *         <li>Parameters: R5+8, R5+12, R5+16, ... (positive offsets)
 *         <li>Return address: R5+4 (saved by CALL instruction)
 *         <li>Old R5: R5+0 (saved by callee prologue)
 *         <li>Local variables: R5-4, R5-8, R5-12, ... (negative offsets)
 *       </ul>
 *   <li><b>Caller Responsibilities:</b> Push arguments, call function, clean up arguments
 *   <li><b>Callee Responsibilities:</b> Save R5, allocate locals, restore R5, return
 * </ul>
 *
 * <p><b>Algorithm: Syntax-Directed Translation</b>
 *
 * <p>This code generator uses a <b>syntax-directed translation</b> approach, which is a form of
 * tree traversal where code generation actions are associated with grammar rules. The algorithm
 * works as follows:
 *
 * <ol>
 *   <li><b>Tree Traversal:</b> Recursively traverse the parse tree in depth-first order
 *   <li><b>Rule Matching:</b> For each nonterminal node, identify the grammar rule that produced it
 *   <li><b>Code Generation:</b> Emit FRISC instructions according to the semantic action associated
 *       with that grammar rule
 *   <li><b>Context Propagation:</b> Pass code generation context (symbol table, activation record,
 *       labels) down the tree
 *   <li><b>Result Assembly:</b> Combine code fragments from child nodes into complete instructions
 * </ol>
 *
 ...
 *       helpers. This avoids generating unused helper code.
 *   <li><b>Float Helpers Before Integer Helpers:</b> Float helpers may mark integer helpers as
 *       needed during their generation, so float helpers must be generated first to properly mark
 *       integer helpers as needed.
 *   <li><b>Global Variables Last:</b> Global variables are generated at the end because they form
 *       the data section, which is conventionally placed after code sections.
 *   <li><b>Immutable Context:</b> The {@link CodeGenContext} is immutable, allowing safe recursive
 *       traversal without side effects on parent contexts.
 * </ul>
 *
 * <p><b>Complexity:</b>
 *
 ...
 *   <li><b>Space Complexity:</b> O(n) for the parse tree + O(m) for generated code buffer, where m
 *       is the number of generated instructions
 * </ul>
 *
 * <p>The code generator implements the PPJ-C to FRISC mapping according to the specification in
 * ppj-labos-upute chapter 5, ensuring compatibility with the FRISC simulator.
 *
...
public final class CodeGenerator {
   ...
   *
   * <ol>
   *   <li>Initializes the FRISC emitter and label generator
   *   <li>Generates program initialization code
   *   <li>Processes all function definitions
   *   <li>Declares global variables and constants
   *   <li>Writes the final assembly to {@code a.frisc}
   * </ol>
   *
   ...
   *
   * <p>This is the main code generation method that orchestrates the entire pipeline. It performs
   * the following steps in order:
   *
   * <ol>
   *   <li><b>Initialization:</b> Creates the emitter, label generator, and initial context
   *   <li><b>Program Entry:</b> Generates the program initialization sequence
   *   <li><b>Translation Unit Processing:</b> Traverses the parse tree to generate function code
   *       and identify needed helper functions
   *   <li><b>Helper Function Generation:</b> Generates helper functions in the correct order (float
   *       helpers first, then integer helpers)
   *   <li><b>Code Emission:</b> Writes all generated code to the output file
   * </ol>
   *
   * <p><b>Preconditions:</b>
   *
   * <ul>
   *   <li>{@code globalScope} must be a valid symbol table from semantic analysis
   *   <li>{@code parseTree} must be a valid parse tree with semantic annotations
   *   <li>{@code outputPath} must be a valid file path (parent directory must exist)
   * </ul>
   *
   * <p><b>Postconditions:</b>
   *
   * <ul>
   *   <li>A complete FRISC assembly file is written to {@code outputPath}
   *   <li>The file contains valid FRISC instructions that can be executed by the simulator
   *   <li>All helper functions needed by the program are included
   * </ul>
   *
   * <p><b>Side Effects:</b>
   *
   * <ul>
   *   <li>Creates or overwrites the file at {@code outputPath}
   *   <li>Modifies the emitter's internal buffer with generated instructions
   *   <li>Updates label generator counters for unique label generation
   * </ul>
   *
   ...
   * @throws CodeGenerationException if code generation fails (I/O errors, unsupported constructs,
   *     etc.)
   */
  public void generate(SymbolTable globalScope, NonTerminalNode parseTree, Path outputPath) {
    ...
    try {
      ...
      CodeGenContext context =
          new CodeGenContext(globalScope, emitter, labelGen, null, null, null, null);
      ...
      if (emitter.needsAnyFloatHelper()) {
        ...
        floatHelperGen.generateFloatHelpers(
            context, flags[0], flags[1], flags[2], flags[3], flags[4], flags[5], flags[6]);
   ...
   *
   * <ol>
   *   <li>Initializes the stack pointer (R7) to 40000
   *   <li>Calls the main function (F_MAIN)
   *   <li>Halts execution (R6 contains the return value)
   * </ol>
   *
   * <p><b>FRISC Semantics:</b>
   *
   * <ul>
   *   <li>Stack pointer (R7) initialized to 40000 (high memory address)
   *   <li>Stack grows downward (decreasing addresses)
   *   <li>CALL instruction saves return address on stack
   *   <li>HALT terminates execution; R6 value is the program result
   * </ul>
   *
   * <p>Generated code:
   *
   ...
   *       <ul>
   *         <li>Traverses the parse tree to find all function definitions
   *         <li>For each function, generates prologue, body, and epilogue
   *         <li>During function code generation, tracks which helper functions are needed (via
   *             emitter flags set by expression generators)
   *       </ul>
   *   <li><b>Second Pass - Global Variable Generation:</b>
   *       <ul>
   *         <li>Extracts all global variable declarations from the parse tree
   *         <li>Generates data declarations (DW for initialized, `DS for uninitialized)
   *         <li>Extracts and emits initializer values for initialized globals
   *       </ul>
   * </ol>
   *
   ...
   *       <ul>
   *         <li>Functions may reference global variables, but forward references are allowed in
   *             FRISC assembly (labels can be used before definition)
   *         <li>This ordering matches conventional assembly layout (code section before data
   *             section)
   *       </ul>
   *   <li><b>Globals Last:</b> Global variables form the data section, which is conventionally
   *       placed at the end of the assembly file
   * </ul>
   *
   * <p><b>Side Effects:</b>
   *
   * <ul>
   *   <li>Emits function code to the emitter buffer
   *   <li>Sets emitter flags indicating which helper functions are needed
   *   <li>Emits global variable declarations to the emitter buffer
   * </ul>
   *
