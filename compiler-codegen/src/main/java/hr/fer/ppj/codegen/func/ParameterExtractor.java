 ...
 *
 * <p>This class handles extraction of parameter lists and parameter names from function declarator
 * nodes.
 *
 * <p><b>Grammar Rules Handled:</b>
 *
 * <ul>
 *   <li>{@code <lista_parametara> ::= <deklaracija_parametra>}
 *   <li>{@code <lista_parametara> ::= <lista_parametara> ZAREZ <deklaracija_parametra>}
 * </ul>
 *
...
public final class ParameterExtractor {
   ...
   *
   * <p>For simple parameters (<ime_tipa> IDN), the IDN is child 1. For complex parameters
   * (<ime_tipa> <deklarator>), we need to search the declarator, not the type specification, to
   * avoid finding the struct tag name instead of the parameter name.
   *
  ...
  private void extractParameterFromDeclaration(NonTerminalNode declaration, List<String> names) {
    ...
    if (children.size() >= 2) {
      ...
      // This avoids finding the struct tag name (e.g., "Outer") instead of the parameter name
      // (e.g., "o")
