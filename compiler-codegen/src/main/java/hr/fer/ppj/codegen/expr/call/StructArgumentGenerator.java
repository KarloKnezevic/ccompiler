 ...
 *
 * <p>This class handles the generation of code for passing struct arguments to functions,
 * implementing the <b>struct argument passing algorithm</b> that copies structs word-by-word onto
 * the stack.
 *
 ...
 *   <li><b>Address Resolution:</b> Compute address of source struct
 *   <li><b>Word-by-Word Copy:</b> Copy struct onto stack word-by-word using a loop
 * </ol>
 *
 * <p><b>Calling Convention:</b>
 *
 * <p>Struct arguments are passed by value, which means the entire struct is copied onto the stack.
 * The struct is pushed word-by-word starting from offset 0, so that:
 *
 * <ul>
 *   <li>First word (offset 0) is pushed first → ends up at lowest address (R5+8)
 *   <li>Last word (offset N) is pushed last → ends up at highest address
 * </ul>
 *
 * <p>This ensures that when the callee accesses the struct via positive offsets from R5, field 0 is
 * at the lowest address, matching the struct layout.
 *
 ...
 * calculate struct size. This class uses {@link StructArraySizeExtractor} to extract array sizes
...
public final class StructArgumentGenerator {
   ...
   *
   * <p>Struct arguments are passed by value, so we need to copy the entire struct onto the stack
   * word-by-word. We push words individually starting from offset 0 and moving forward through the
   * struct.
   *
   * <p>The callee expects the struct with field 0 at the lowest address (R5+8 for the first
   * parameter). When we push from offset 0 forward:
   *
   * <ul>
   *   <li>First word (offset 0) is pushed first → ends up at lowest address (R5+8) ✓
   *   <li>Last word (offset N) is pushed last → ends up at highest address ✓
   ...
   *     structs with arrays)
   */
  public void generateStructArgument(
      NonTerminalNode argExpr,
