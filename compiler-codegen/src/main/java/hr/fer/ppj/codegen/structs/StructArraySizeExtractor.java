 ...
 *
 * <p>This class searches the parse tree to find struct definitions and extracts array size
 * information for array fields. This is needed because {@code StructType} doesn't preserve array
 * sizes - it only stores {@code ArrayType(elementType)}.
 *
 * <p><b>Why This Class Exists:</b>
 *
 * <p>The semantic analyzer's type system stores struct field types as {@code ArrayType}, but {@code
 * ArrayType} only stores the element type, not the array size. The array size is only available in
 * the parse tree (from the declaration syntax: {@code int arr[10]}).
 *
 * <p>This class bridges that gap by:
 *
 * <ul>
 *   <li>Searching the parse tree for struct definitions
 *   <li>Matching struct definitions by tag name
 *   <li>Extracting array sizes from field declarations
 ...
 *
 * <ul>
 *   <li>{@code <struct_specifikator>} - struct type specifier
 *   <li>{@code <struct_lista_deklaracija>} - struct field declaration list
 *   <li>{@code <struct_deklaracija>} - struct field declaration
 *   <li>{@code <struct_deklarator>} - struct field declarator
 *   <li>{@code <izravni_deklarator>} - direct declarator (with array brackets)
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li><b>Semantic Attributes First:</b> Checks semantic attributes on declarator nodes (fastest,
 *       most reliable if available)
 *   <li><b>Tag-Based Search:</b> Searches for struct definitions matching the struct tag
 *   <li><b>Fallback Search:</b> If tag-based search fails, searches all struct definitions (useful
 *       for anonymous structs or when tag matching fails)
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li>Each struct is extracted independently by its tag
 *   <li>Nested structs are found when searching the parse tree recursively
 *   <li>Array sizes for nested struct fields are extracted when processing the nested struct
 * </ul>
 *
 * <p><b>Example:</b>
 *
 ...
 *
 * <ul>
 *   <li>Struct tag name for tagged structs
 *   <li>{@code "__anonymous__"} for anonymous structs
 * </ul>
 *
 * <p><b>Limitations:</b>
 *
 * <ul>
 *   <li>Requires parse tree to be available (may be null in some contexts)
 *   <li>Array sizes must be compile-time constants (BROJ terminals)
 *   <li>Anonymous structs are harder to match (fallback search may match wrong struct)
 * </ul>
 *
 * <p><b>Complexity Analysis:</b>
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(n) where n is the number of nodes in the parse tree (worst case:
 *       full tree traversal)
 *   <li><b>Space Complexity:</b> O(m) where m is the number of structs with arrays (for caching)
 * </ul>
 *
...
public final class StructArraySizeExtractor {
   ...
   *
   * <p>This method searches the parse tree for a struct definition matching the given tag and
   * extracts array sizes for all array fields in that struct.
   *
   * <p><b>Extraction Strategy:</b>
   *
   * <ol>
   *   <li><b>Cache Check:</b> Returns cached result if available
   *   <li><b>Tag-Based Search:</b> Searches for struct definition with matching tag
   *   <li><b>Fallback Search:</b> If tag-based search fails, searches all struct definitions
   *   <li><b>Cache Result:</b> Stores result in cache for future lookups
   * </ol>
   *
   * <p><b>Return Value:</b>
   *
   * <ul>
   *   <li>Returns a map from field name to array size (only for array fields)
   *   <li>Non-array fields are not included in the map
   *   <li>Empty map means either: struct has no arrays, or struct not found
   * </ul>
   *
   * <p><b>Example:</b>
   *
  ...
  /**
   * Extracts array sizes from all struct definitions in the parse tree. Used as a fallback when
   * tag-based search fails.
   */
  ...
  private void extractArraySizesFromNodeRecursive(
      NonTerminalNode node, Map<String, Integer> result) {
  ...
  private Map<String, Integer> extractArraySizesFromNode(NonTerminalNode node, String structTag) {
    ...
    // Look for struct specifier: KR_STRUCT IDN L_VIT_ZAGRADA <struct_lista_deklaracija>
    // D_VIT_ZAGRADA
    ...
    for (ParseNode child : node.children()) {
      if (child instanceof NonTerminalNode childNode) {
        ...
        // AND we're looking for a specific tag AND the result came from the exact match logic
        // above)
  ...
  private Map<String, Integer> extractArraySizesFromFieldList(NonTerminalNode fieldListNode) {
    ...
    // Field list: <struct_lista_deklaracija> ::= <struct_deklaracija> | <struct_lista_deklaracija>
    // <struct_deklaracija>
  ...
  private Map<String, Integer> extractArraySizesFromDeclaration(NonTerminalNode declNode) {
    ...
    // Declaration: <struct_deklaracija> ::= <lista_specifikatora_kvalifikatora>
    // <struct_lista_deklaratora> TOCKAZAREZ
  ...
  private Map<String, Integer> extractArraySizesFromDeclaratorList(
      NonTerminalNode declaratorListNode) {
    Map<String, Integer> result = new HashMap<>();

    // Check semantic attributes first - might have field info with array sizes
    if (declaratorListNode.attributes() != null
        && declaratorListNode.attributes().structFields() != null) {
    ...
    // Declarator list: <struct_lista_deklaratora> ::= <struct_deklarator> |
    ...
    if (children.size() == 1) {
      ...
      if (declarator instanceof NonTerminalNode declNode
          && "<struct_deklarator>".equals(declNode.symbol())) {
   ...
   *
   * <p>Structure: <struct_deklarator> -> <deklarator> -> <izravni_deklarator> -> IDN L_UGL_ZAGRADA
   * <izraz> D_UGL_ZAGRADA
   *
   * <p>Note: elementCount is set on the nested <deklarator> node, not on <struct_deklarator>
   * itself.
   */
  private Map<String, Integer> extractArraySizesFromDeclarator(NonTerminalNode declaratorNode) {
    ...
    if (children.size() == 1
        && children.get(0) instanceof NonTerminalNode nestedDecl
   ...
   *
   * <p>Structure: <struct_deklarator> -> <deklarator> -> <izravni_deklarator> ->
   * <izravni_deklarator> IDN L_UGL_ZAGRADA <izraz> D_UGL_ZAGRADA
   */
  private Map<String, Integer> extractArraySizesFromDeclaratorRecursive(NonTerminalNode node) {
    ...
    // Pattern 1: <izravni_deklarator> -> <izravni_deklarator> IDN L_UGL_ZAGRADA <izraz>
    // D_UGL_ZAGRADA
    ...
    if ("<izravni_deklarator>".equals(symbol)) {
      ...
      for (int i = 0; i < children.size() - 3; i++) {
        ...
        if (i + 3 < children.size()) {
          ...
          if (idn instanceof TerminalNode idnTerminal
              && "IDN".equals(idnTerminal.symbol())
              && leftBracket instanceof TerminalNode
              && "L_UGL_ZAGRADA".equals(leftBracket.symbol())
              && rightBracket instanceof TerminalNode
   ...
   *
   * <p>This method searches for a {@code BROJ} (number) terminal in the expression tree and parses
   * it as an integer. Array sizes in C must be compile-time constants, so the expression should
   ...
   *
   * <ul>
   *   <li>{@code int arr[10];} - expression is {@code 10} (BROJ terminal)
   *   <li>{@code int arr[2 * 5];} - expression is {@code 2 * 5} (must be constant-folded)
   * </ul>
   *
   * <p><b>Limitation:</b> This method only handles simple {@code BROJ} terminals. Constant-folded
   * expressions (like {@code 2 * 5}) would need to be evaluated by the semantic analyzer first. In
   * practice, array sizes are typically simple constants.
   *
