 ...
 *
 * <p>This class handles the extraction of array dimensions from C declarator syntax, which is
 * needed to calculate the total size of array variables for stack allocation.
 *
 * <p><b>Purpose:</b>
 *
 * <p>When processing local variable declarations like {@code int arr[5];}, we need to extract the
 * array size (5) to calculate the total memory needed (5 * 4 = 20 bytes). This class traverses the
 * declarator AST structure to find array dimension specifications.
 *
 * <p><b>Grammar Rule:</b>
 *
 ...
 *
 * <ol>
 *   <li>Traverse the declarator structure looking for {@code <izravni_deklarator>} nodes
 *   <li>Search for the pattern: {@code L_UGL_ZAGRADA BROJ D_UGL_ZAGRADA}
 *   <li>Extract the numeric value from the {@code BROJ} terminal
 *   <li>Handle nested declarators (for multi-dimensional arrays)
 * </ol>
 *
 * <p><b>Limitations:</b>
 *
 * <p>This class extracts array sizes from the parse tree structure. It defaults to 4 bytes per
 * element (int size) when calculating total size, as it doesn't have access to the type specifier.
 * The actual element size should be determined from semantic attributes when available.
 *
...
public final class ArraySizeExtractor {
   ...
   *
   * <p>Returns 4 (default) for non-arrays, or size * element_size for arrays. Note: This method is
   * used as a fallback when variable is not in activation record. It defaults to 4 bytes per
   * element (int), but ideally should check the type specifier.
   *
   ...
   *
   * <p>Defaults to 4 bytes per element (int). For char arrays, this should be 1 byte, but without
   * access to the type specifier, we default to int.
   *
  ...
  private static int extractArraySizeFromDirectDeclarator(NonTerminalNode directDeclarator) {
    ...
    // For arrays: <izravni_deklarator> -> <izravni_deklarator> -> IDN L_UGL_ZAGRADA BROJ
    // D_UGL_ZAGRADA
   ...
   *
   * <p>Tries to get elementCount from semantic attributes first, then falls back to parsing the
