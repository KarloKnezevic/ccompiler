 ...
 *
 * <p>This class handles the generation of code for computing addresses of array elements, including
 * arrays within structs and nested structs. It implements the <b>array element address calculation
 ...
 *   <li><b>Index Preservation:</b> Save index on stack before computing base address (recursive
 *       base computation may use R0)
 *   <li><b>Base Address Resolution:</b> Recursively compute the base address of the array (handles
 *       field access, nested structs, etc.)
 *   <li><b>Element Size Calculation:</b> Calculate element size (may require array size extraction
 *       for struct elements with arrays)
 *   <li><b>Index Multiplication:</b> Multiply index by element size (optimized for powers of 2)
 *   <li><b>Address Calculation:</b> Add index offset to base address
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li>Evaluate index {@code i}
 *   <li>Save index on stack
 *   <li>Compute base address of {@code o.arr} (recursively handles field access)
 *   <li>Restore index from stack
 *   <li>Multiply index by element size
 *   <li>Add to base address
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>4 bytes (most common):</b> Use {@code SHL index, %D 2, index} (left shift by 2)
 *   <li><b>Power of 2:</b> Use {@code SHL} with appropriate shift amount
 *   <li><b>Other sizes:</b> Use repeated {@code ADD} operations
 * </ul>
 *
 ...
 *   <li><b>Space Complexity:</b> O(1) - uses only registers and stack for index
 * </ul>
 *
...
public final class ArrayElementAddressGenerator {
   ...
   */
  public ArrayElementAddressGenerator(
   ...
   *
   * <p>This method uses PUSH/POP to save the index calculation, ensuring that recursive base
   * address computation doesn't overwrite it.
   *
   * <p>Handles deeply nested array access like:
   *
   * <ul>
   *   <li>{@code a[i]}: simple array
   *   <li>{@code o.arr[i]}: array field in struct
   *   <li>{@code o.middle.inner.arr[0]}: deeply nested struct with array field
   * </ul>
   *
   * @param arrayIndexNode the array indexing node
   * @param targetRegister the register to store the element address in
   * @throws IllegalStateException if the node structure is invalid or element size cannot be
   *     calculated
   */
  public void generateArrayElementAddress(NonTerminalNode arrayIndexNode, String targetRegister) {
    ...
    if (children.size() != 4) {
      throw new IllegalStateException(
          "Invalid array indexing node structure: expected 4 children, got "
              + children.size()
              + " for node "
              + arrayIndexNode.symbol());
    }
    ...
    if (baseType == null) {
      throw new IllegalStateException(
          "Base expression has no type annotation for array indexing. " + "Node: " + base.symbol());
    }

    Type strippedBaseType = TypeSystem.stripConst(baseType);
    if (!(strippedBaseType instanceof ArrayType arrayType)) {
      throw new IllegalStateException(
          "Array indexing base is not an array type: "
              + strippedBaseType
              + " (node: "
              + base.symbol()
              + ")");
    ...
    try {
      elementSize =
          TypeSizeCalculator.calculateTypeSize(strippedElementType, nestedStructArraySizes);
      ...
      if (strippedElementType instanceof StructType structElementType
        ...
        throw new IllegalStateException(
            "Cannot calculate element size for array indexing: "
                + e.getMessage()
                + " (elementType: "
                + elementType
                + ", stripped: "
   ...
   *
   * <ul>
   *   <li>4 bytes: {@code SHL R1, %D 2, R1} (left shift by 2)
   *   <li>Power of 2: {@code SHL} with appropriate shift amount
   *   <li>Other sizes: Repeated {@code ADD} operations
   * </ul>
   *
  ...
  private void multiplyIndexByElementSize(int elementSize) {
    if (elementSize == 4) {
      ...
      context
          .emitter()
          .emitInstruction(
              "SHL", "R1", "%D " + shiftAmount, "R1", "index * " + elementSize + " (element size)");
    } else {
      ...
      for (int i = 1; i < elementSize; i++) {
        context.emitter().emitInstruction("ADD", tempReg, "R1", tempReg, "index * " + (i + 1));
      }
