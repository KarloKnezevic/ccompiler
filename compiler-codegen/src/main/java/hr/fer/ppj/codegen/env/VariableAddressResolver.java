 ...
 *
 * <p>This class provides methods to get the FRISC address expression for variables, handling both
 * local variables (in activation records) and global variables (via labels). It implements the
 * <b>variable address resolution algorithm</b> used throughout code generation.
 *
 ...
 *
 * <ul>
 *   <li><b>Local Variable Strategy:</b> Uses frame pointer (R5) with negative offsets
 *   <li><b>Global Variable Strategy:</b> Uses global labels (G_<NAME>)
 * </ul>
 *
 * <p>The strategy is chosen automatically based on whether the variable is found in the current
 ...
 *
 * <ol>
 *   <li><b>Check Function Context:</b> Determine if we're in a function (have an activation record)
 *   <li><b>Check Local Scope:</b> If in a function, check if the variable exists in the activation
 *       record (local variable or parameter)
 *   <li><b>Local Variable Resolution:</b> If found locally:
 *       <ul>
 *         <li>Get the variable's offset from the activation record
 *         <li>Format as FRISC address: {@code (R5+offset)} for parameters or {@code (R5-offset)}
 *             for locals
 *       </ul>
 *   <li><b>Global Variable Resolution:</b> If not found locally:
 *       <ul>
 *         <li>Generate global variable label using LabelGenerator
 *         <li>Format as FRISC address: {@code (G_<VARIABLE_NAME>)}
 *       </ul>
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>Parameters:</b> {@code (R5+08)}, {@code (R5+0C)}, etc. (positive offsets)
 *   <li><b>Local Variables:</b> {@code (R5-04)}, {@code (R5-08)}, etc. (negative offsets)
 * </ul>
 *
 * <p><b>Global Variables:</b>
 *
 * <ul>
 *   <li><b>Format:</b> {@code (G_<VARIABLE_NAME>)} where G_<VARIABLE_NAME> is the label
 *   <li><b>Example:</b> Variable {@code x} → {@code (G_X)}
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Local Scope First:</b> Local variables shadow global variables with the same name
 *   <li><b>Frame Pointer Access:</b> Local variables are accessed via frame pointer (R5) because
 *       they're allocated on the stack
 *   <li><b>Label Access:</b> Global variables are accessed via labels because they're allocated in
 *       the data section
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Stack:</b> Local variables and function parameters (accessed via R5)
 *   <li><b>Data Section:</b> Global variables (accessed via labels)
 *   <li><b>Code Section:</b> Instructions (accessed via labels for functions)
 * </ul>
 *
 * <p><b>Example:</b>
 *
 ...
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(1) for local variables (hash map lookup), O(1) for global
 *       variables (label generation)
 *   <li><b>Space Complexity:</b> O(1) - uses only existing data structures
 * </ul>
 *
...
public final class VariableAddressResolver {
   ...
   *
   * <p>This method checks if the variable is local (in the current function's activation record) or
   * global, and returns the appropriate address expression.
   *
   * <p><b>Key invariant:</b> Parameters are ALWAYS accessed via R5+offset, never via global labels.
   * This method ensures that if a variable exists in the activation record (whether parameter or
   * local), it uses the activation record address, not a global label.
   *
   ...
   *
   * <p>This method checks if the expression is a simple identifier (variable reference) and returns
   * the variable name. It does NOT extract field names from field access expressions (p.x should
   * return null, not "p").
   *
