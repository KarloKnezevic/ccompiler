 ...
 *
 * <ul>
 *   <li>Simple variables (IDN)
 *   <li>Array elements (a[i])
 *   <li>Struct fields (p.x, o.inner.value)
 *   <li>Nested combinations (o.arr[i])
 * </ul>
 *
 * <p><b>Key Invariant:</b> This generator computes ADDRESSES, not values. For struct variables, it
 * computes the address of the struct object itself (e.g., R5-8 for a local struct), NOT the value
 * stored at that address.
 *
 * <p><b>Algorithm:</b>
 *
 * <ol>
 *   <li>If simple variable: compute address from frame pointer or global label
 *   <li>If field access: recursively compute base address, then add field offset
 *   <li>If array indexing: compute base address, then add index * elementSize
 * </ol>
 *
...
public final class LValueAddressGenerator {
   ...
   *
   * <ul>
   *   <li>Simple variables: {@code x} → address of x
   *   <li>Field access: {@code p.x} → address of field x in struct p
   *   <li>Nested field access: {@code o.inner.value} → address of nested field
   *   <li>Array indexing: {@code a[i]} → address of array element
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>Simple variables: {@code x}
   *   <li>Field access: {@code p.x}, {@code o.inner.value}
   *   <li>Array indexing: {@code a[i]}, {@code o.arr[i]}
   *   <li>Deeply nested: {@code o.middle.inner.arr[0]}
   * </ul>
   *
   * <p>The algorithm recursively processes each level:
   *
   * <ol>
   *   <li>If field access: recursively compute base address, add field offset
   *   <li>If array indexing: recursively compute base address, add index * elementSize
   *   <li>If simple variable: compute variable address
   * </ol>
   *
  ...
  public void generateAddress(NonTerminalNode node, String targetRegister) {
    ...
    if (LValuePatternMatcher.isFieldAccess(unwrapped)) {
      ...
      StructFieldAddressGenerator fieldGen =
          new StructFieldAddressGenerator(context, expressionGenerator, this, arraySizeExtractor);
    ...
    if (LValuePatternMatcher.isArrayIndexing(unwrapped)) {
      // Use specialized array element address generator (handles null arraySizeExtractor
      // gracefully)
      ArrayElementAddressGenerator arrayGen =
          new ArrayElementAddressGenerator(context, expressionGenerator, this, arraySizeExtractor);
    ...
    if (variableName != null) {
      ...
      // This is especially important for distinguishing struct-by-value parameters from
   ...
   *
   * <p>For local variables, computes: targetRegister = R5 + offset For global variables, computes:
   * targetRegister = G_LABEL
   *
   * <p><b>Key:</b> This computes the ADDRESS of the variable, not its value. For struct variables,
   * this is the address of the struct object itself.
   *
   * <p><b>Array Parameters:</b> In C, array parameters decay to pointers. When accessing an array
   * parameter (e.g., {@code a[3]} where {@code a} is a parameter), we need to:
   *
   * <ol>
   *   <li>Compute the address of the parameter slot (e.g., R5+8)
   *   <li>LOAD the pointer value from that slot (the actual array base address)
   * </ol>
   *
   ...
   */
  private void generateVariableAddress(
      String variableName, String targetRegister, NonTerminalNode expressionNode) {
   ...
   *
   * <p>Returns null if the expression is not a simple variable (e.g., if it's a field access or
   * array indexing).
   *
  ...
  private String extractVariableName(NonTerminalNode node) {
    ...
    if (LValuePatternMatcher.isFieldAccess(unwrapped)
        || LValuePatternMatcher.isArrayIndexing(unwrapped)) {
      return null;
    }
