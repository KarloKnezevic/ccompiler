 ...
 * <ol>
 *   <li>Extract initializer values from the parse tree</li>
 *   <li>Generate code to store each value at the appropriate array element address</li>
 *   <li>Zero-initialize any remaining elements (C standard: partial initializers zero-fill)</li>
 * </ol>
 * 
 ...
 * <ul>
 *   <li>Base offset is the lowest address (furthest from R5)</li>
 *   <li>Element i is at: {@code baseOffset + (i * elementSize)}</li>
 *   <li>For example, {@code int x[3]} at R5-0C (-12): 
 *       x[0] at R5-0C, x[1] at R5-08 (-8), x[2] at R5-04 (-4)</li>
 * </ul>
 * 
 * <p><b>Grammar Rule:</b>
 * 
...
public final class ArrayInitializerGenerator {
     ...
     * </pre>
     * 
     * <p>For partial initializers (e.g., {@code int x[3] = { 51 };}), after storing
     * the explicit values, zero-initializes the remaining elements.
     * 
     ...
     */
    public void generate(String varName, String baseAddress, NonTerminalNode initializer,
                       int arraySize, int elementSize) {
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            if (i < numInitialized) {
                ...
                context.emitter().emitInstruction("MOVE", "%D " + cleanValue, "R0", 
                    "initialize " + varName + "[" + i + "]");
                context.emitter().emitInstruction("STORE", "R0", elementAddress, 
                    "store " + varName + "[" + i + "]");
            } else {
                // Zero-initialize remaining elements (C standard: partial initializers zero-fill)
                context.emitter().emitInstruction("MOVE", "%D 0", "R0", 
                    "zero-initialize " + varName + "[" + i + "]");
                context.emitter().emitInstruction("STORE", "R0", elementAddress, 
                    "store " + varName + "[" + i + "]");
            }
        }
     ...
     * 
     * <p>For now, only supports simple constant literals (integers).
     * More complex expressions would need to be evaluated at runtime.
     * 
