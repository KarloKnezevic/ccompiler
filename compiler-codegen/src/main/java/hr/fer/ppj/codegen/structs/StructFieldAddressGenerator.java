 ...
 *
 * <p>This class handles the generation of code for computing addresses of struct fields, including
 * nested field access. It implements the <b>struct field address calculation algorithm</b> used
 ...
 *       (handles nested field access like {@code o.middle.inner})
 *   <li><b>Field Offset Lookup:</b> Get the byte offset of the field within the struct using {@link
 *       StructFieldOffsetCalculator}
 *   <li><b>Address Calculation:</b> Add field offset to base address: {@code ADD targetRegister, %D
 *       fieldOffset, targetRegister}
 * </ol>
 *
 ...
 * </ol>
 *
 * <p>This is handled recursively by delegating to the main {@link LValueAddressGenerator} for the
 * base expression, which may itself be a field access.
 *
 * <p><b>Array Size Extraction:</b>
 *
 * <p>For structs containing arrays, array sizes must be extracted from the parse tree to correctly
 * calculate field offsets. This class uses {@link StructArraySizeExtractor} to extract array sizes
 * for the current struct and all nested structs.
 *
...
public final class StructFieldAddressGenerator {
   ...
   */
  public StructFieldAddressGenerator(
   ...
   *
   * <ul>
   *   <li>{@code p.x}: compute address of p, add offset of x
   *   <li>{@code o.inner.value}: recursively compute address of o.inner, add offset of value
   *   <li>{@code o.middle.inner.arr}: recursively handles arbitrary nesting depth
   * </ul>
   *
  ...
  public void generateFieldAddress(NonTerminalNode fieldAccessNode, String targetRegister) {
    ...
    if (children.size() != 3) {
      throw new IllegalStateException(
          "Invalid field access node structure: expected 3 children, got "
              + children.size()
              + " for node "
              + fieldAccessNode.symbol());
    }
    ...
    if (baseType == null) {
      throw new IllegalStateException(
          "Base expression has no type annotation. "
              + "Node: "
              + base.symbol()
              + ", field: "
              + fieldName);
    ...
    if (!(strippedBaseType instanceof StructType structType)) {
      throw new IllegalStateException(
          "Field access base is not a struct type: "
              + strippedBaseType
              + " (field: "
              + fieldName
              + ")");
