 ...
 *
 * <p>This utility class provides methods to extract type information from type-related parse tree
 * nodes, such as {@code <ime_tipa>} and {@code <specifikator_tipa>}.
 *
 * <p><b>Grammar Rules Handled:</b>
 *
 * <ul>
 *   <li>{@code <ime_tipa>} - type name nodes
 *   <li>{@code <specifikator_tipa>} - type specifier nodes (KR_CHAR, KR_INT, etc.)
 * </ul>
 *
 ...
 *       (preferred, as it's more reliable and includes struct types)
 *   <li><b>Tree Parsing Fallback:</b> Falls back to parsing the tree structure to find type
 *       specifier terminals (KR_CHAR, KR_INT, etc.)
 * </ol>
 *
 * <p><b>Struct Type Support:</b>
 *
 * <p>For struct types, the semantic attributes approach is essential, as struct types cannot be
 * determined by parsing terminals alone. The semantic analyzer stores the complete struct type
 * information (including field types, nested structs, etc.) in the semantic attributes.
 *
 * <p><b>Limitations:</b>
 *
 ...
 *
 * <ul>
 *   <li>Struct types require semantic analysis to resolve struct tags and field information
 *   <li>Array types require size information that's not in the parse tree
 *   <li>Pointer types require base type information
 * </ul>
 *
...
public final class TypeNodeExtractor {
   ...
   *       (preferred for struct types and other complex types)
   *   <li><b>Tree Parsing:</b> Falls back to parsing the tree structure to find type specifier
   *       terminals (KR_CHAR, KR_INT)
   * </ol>
   *
   * <p><b>Struct Type Handling:</b>
   *
   * <p>For struct type nodes, the semantic attributes approach is essential. The semantic analyzer
   * stores the complete {@link hr.fer.ppj.semantics.types.StructType} information in the
   * attributes, including:
   *
   * <ul>
   *   <li>Struct tag
   *   <li>Field names and types
   *   <li>Nested struct information
   * </ul>
   *
   * <p>The tree parsing fallback cannot extract struct types, as they require semantic analysis to
   ...
   *
   * <p>This method recursively searches the parse tree for type specifier terminals. It only works
   * for primitive types (char, int) because:
   *
   * <ul>
   *   <li>Struct types require semantic analysis to resolve struct definitions
   *   <li>Array types require size information not in the parse tree
   *   <li>Pointer types require base type information
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>{@code KR_CHAR} - returns {@link PrimitiveType#CHAR}
   *   <li>{@code KR_INT} - returns {@link PrimitiveType#INT}
   * </ul>
   *
