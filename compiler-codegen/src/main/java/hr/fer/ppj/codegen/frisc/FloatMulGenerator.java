 ...
 *
 * <p>This function implements <b>Q16.16 fixed-point multiplication</b> using a <b>64-bit signed
 * product</b> computed via the Russian peasant (shift-and-add) algorithm with HI:LO register pairs.
 * The full 64-bit product is then shifted right by 16 bits to obtain the Q16.16 result. This
 * ensures correct results for all values in the safe domain, including small decimal numbers like
 * 1.5 × 2.5 = 3.75.
 *
 * <h2>1. Q16.16 Representation</h2>
 *
 * <p>In this compiler, the C type {@code float} is <b>not</b> IEEE-754. It is implemented as a
 * <b>signed 32-bit Q16.16 fixed-point value</b>:
 *
 * <ul>
 *   <li>Internally stored as a 32-bit signed integer {@code raw}
 *   <li>Real value: {@code value = raw / 65536.0} (i.e. {@code raw / 2^16})
 *   <li>Bits 31-16: Integer part (signed 16-bit, range -32768 to 32767)
 ...
 *
 * <ul>
 *   <li>Minimum: {@code raw = -2^31} → {@code value = -2^31 / 2^16 = -32768.0}
 *   <li>Maximum: {@code raw = 2^31 - 1} → {@code value ≈ (2^31 - 1) / 2^16 ≈ 32767.9999847}
 * </ul>
 *
 ...
 * <blockquote>
 *
 * <b>Any C {@code float} between approximately -32768.0 and +32767.9999 is representable in
 * Q16.16.</b>
 *
 * </blockquote>
 *
 ...
 *
 * <ul>
 *   <li>{@code raw_a} (32-bit signed) representing {@code A = raw_a / 65536.0}
 *   <li>{@code raw_b} (32-bit signed) representing {@code B = raw_b / 65536.0}
 * </ul>
 *
 ...
 * </pre>
 *
 * <p><b>Critical Requirement:</b> The multiplication {@code raw_a * raw_b} <b>must be a full 64-bit
 * signed product</b>, implemented with two 32-bit FRISC registers {@code (HI : LO)}. This
 * guarantees correct results for small decimal numbers like:
 *
 * <ul>
 *   <li>{@code 1.5 × 2.5 = 3.75} → raw = 3.75 × 65536 = 245760
 *   <li>{@code 1.5 × 2.5 × 3.5 = 13.125} → raw = 860160
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>The product {@code raw_a * raw_b} can exceed 32 bits even for small values
 *   <li>For example: {@code 1.5 × 2.5} in Q16.16 is {@code 98304 × 163840 = 16,106,127,360}
 *   <li>This value (0x3C0000000) requires 34 bits, so a 32-bit multiply would wrap around
 *   <li>After wrapping, shifting right by 16 produces an incorrect result
 * </ul>
 *
 * <p>By computing the <b>full 64-bit product</b> and then shifting right by 16 bits, we ensure that
 * the correct middle 32 bits are extracted, producing accurate Q16.16 results.
 *
 ...
 *
 * <ol>
 *   <li>Load raw {@code a} and {@code b} from the stack (Q16.16, signed 32-bit)
 *   <li>If either operand is zero, return 0 immediately
 *   <li>Initialize a sign register: {@code sign = +1}
 *   <li>If {@code a < 0}: negate {@code a} and flip sign: {@code sign = -sign}
 *   <li>If {@code b < 0}: negate {@code b} and flip sign: {@code sign = -sign}
 *   <li>After this: {@code a_abs = |a|}, {@code b_abs = |b|}, {@code sign = +1 or -1}
 * </ol>
 *
 * <h3>Step 2: 64-bit Russian Peasant Multiplication</h3>
 *
 * <p>We now multiply two <b>non-negative 32-bit integers</b> using the Russian peasant algorithm,
 * but with <b>64-bit accumulation</b> using HI:LO register pairs.
 *
 * <p><b>Register Allocation:</b>
 *
 * <ul>
 *   <li>{@code R0 = M_LO} (64-bit multiplicand low, initially {@code |a|})
 *   <li>{@code R3 = M_HI} (64-bit multiplicand high, initially 0)
 *   <li>{@code R1 = B} (multiplier, initially {@code |b|})
 *   <li>{@code R2 = ACC_LO} (64-bit accumulator low)
 *   <li>{@code R4 = ACC_HI} (64-bit accumulator high, sign saved on stack)
 *   <li>{@code R6 = temp} (for bit checks and carry extraction)
 * </ul>
 *
 ...
 *       <ul>
 *         <li>{@code ACC_LO = ACC_LO + M_LO} (using {@code ADD})
 *         <li>{@code ACC_HI = ACC_HI + M_HI + carry} (using {@code ADC})
 *       </ul>
 *   <li>Perform 64-bit left shift: {@code M <<= 1}:
 *       <ul>
 *         <li>Save old {@code M_LO}
 *         <li>{@code M_LO <<= 1}
 *         <li>Extract carry bit from old {@code M_LO} (bit 31): {@code carry = (old_M_LO >> 31) &
 *             1}
 *         <li>{@code M_HI <<= 1}
 *         <li>{@code M_HI |= carry}
 *       </ul>
 *   <li>{@code B >>= 1} (logical right shift)
 * </ol>
 *
 ...
 *
 * <p>We have the 64-bit product in {@code (ACC_HI, ACC_LO)}. To convert to Q16.16, we need to
 * extract the middle 32 bits:
 *
 * <pre>
 * raw_result_abs = (ACC_HI << 16) | (ACC_LO >> 16)
 * </pre>
 *
 * <p>This is equivalent to shifting the 64-bit product right by 16 bits and taking the lower 32
 * bits of the result.
 *
 ...
 *
 * <ul>
 *   <li>Adding/subtracting 32-bit words
 *   <li>Negating operands (two's complement: {@code 0 - x})
 *   <li>Incrementing/decrementing loop counters
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Multiplicand doubling:</b> {@code M_LO <<= 1}, {@code M_HI <<= 1}
 *   <li><b>Multiplier halving:</b> {@code B >>= 1}
 *   <li><b>Carry extraction:</b> {@code (old_M_LO >> 31) & 1} to extract bit 31
 *   <li><b>Q16.16 scaling:</b> {@code ACC_LO >> 16} and {@code ACC_HI << 16}
 *   <li><b>Bit combination:</b> {@code OR} to combine shifted values
 * </ul>
 *
 ...
 * </pre>
 *
 * <p>where {@code carry} comes from the carry flag (C) set by a previous {@code ADD} or {@code SUB}
 * instruction. This is <b>essential</b> for correctly implementing 64-bit addition using two 32-bit
 * registers.
 *
 ...
 *
 * <ol>
 *   <li>Add the low 32 bits first: {@code ACC_LO = ACC_LO + M_LO}
 *   <li>This may produce a carry (overflow) if {@code ACC_LO + M_LO ≥ 2^32}
 *   <li>The carry flag (C) is automatically set by the {@code ADD} instruction
 *   <li>Add the high 32 bits with the carry: {@code ACC_HI = ACC_HI + M_HI + C}
 * </ul>
 *
 ...
 * </pre>
 *
 * <p>This ensures correct propagation of the carry from the low word to the high word, exactly as
 * required in 64-bit arithmetic. Without {@code ADC}, the carry would be lost, leading to incorrect
 * results for products that exceed 32 bits.
 *
 * <p><b>Example:</b>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>It is not used in {@code F_FMUL} (which only uses addition), but may be used in other float
 * operations like division for multiword subtraction.
 *
 ...
 * <blockquote>
 *
 * <b>Safe domain for Q16.16 multiplication without overflow for any pair of operands:</b> <br>
 * each operand in <b>[-181.0, +181.0]</b>.
 *
 * </blockquote>
 *
 ...
 *
 * <ul>
 *   <li>Q16.16 can still represent the individual values
 *   <li>But the product may overflow the 32-bit result and wrap around (two's complement)
 * </ul>
 *
 * <p><b>Note:</b> For typical operations (e.g., {@code 1.5 * 2.5 * 3.5}), the algorithm is exact
 * and overflow-free. The 64-bit product ensures correct results even when the intermediate product
 * exceeds 32 bits.
 *
 * <h2>6. Complexity Analysis</h2>
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(32) - the Russian peasant loop iterates at most 32 times (once
 *       per bit in the multiplier)
 *   <li><b>Space Complexity:</b> O(1) - uses only registers (no dynamic memory allocation)
 * </ul>
 *
 * <h2>7. FRISC Register Usage</h2>
 *
 * <ul>
 *   <li><b>R0:</b> M_LO (multiplicand low), then result after scaling
 *   <li><b>R1:</b> B (multiplier), then ACC_HI during scaling
 *   <li><b>R2:</b> ACC_LO (accumulator low)
 *   <li><b>R3:</b> M_HI (multiplicand high), then temporary for negation
 *   <li><b>R4:</b> Sign flag (+1 or -1), then ACC_HI (accumulator high)
 *   <li><b>R5:</b> Frame pointer (preserved)
 *   <li><b>R6:</b> Temporary during loop, then return value (Q16.16 result)
 *   <li><b>R7:</b> Stack pointer (preserved)
 *   <li><b>Stack:</b> Used to save sign flag during multiplication
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>{@code a = 1.5} → {@code raw_a = 98304} (0x00018000)
 *   <li>{@code b = 2.5} → {@code raw_b = 163840} (0x00028000)
 * </ul>
 *
 * <p><b>Computation:</b>
 *
 * <ol>
 *   <li>Both operands are positive, so {@code sign = +1}, {@code a_abs = 98304}, {@code b_abs =
 *       163840}
 *   <li>64-bit multiplication: {@code 98304 × 163840 = 16,106,127,360} (0x3C0000000)
 *   <li>64-bit product: {@code (HI, LO) = (0x00000003, 0xC0000000)}
 *   <li>Extract middle 32 bits: {@code (0x00000003 << 16) | (0xC0000000 >> 16) = 0x0003C000 =
 *       245760}
 *   <li>Apply sign: {@code result = +245760}
 * </ol>
 *
 * <p><b>Output:</b>
 *
 * <ul>
 *   <li>{@code raw_result = 245760} (0x0003C000)
 *   <li>{@code value = 245760 / 65536 = 3.75} ✓
 * </ul>
 *
...
public final class FloatMulGenerator {
  ...
  public void generate(CodeGenContext context) {
    ...
    emitter.emitComment(
        "F_FMUL: Q16.16 fixed-point multiplication using 32-bit Russian peasant algorithm");
    ...
    emitter.emitInstruction(
        "SHR", "R0", "%D 16", "R0", "R0 = ACC_LO >> 16 (low 16 bits of result)");
