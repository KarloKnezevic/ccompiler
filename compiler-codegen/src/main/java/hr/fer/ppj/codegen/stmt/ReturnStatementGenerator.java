 ...
 *
 * <p>This class handles both return statements with values and void returns, optimizing simple
 * literal returns by generating them directly into R6.
 *
...
public final class ReturnStatementGenerator {
  ...
  /**
   * Attempts to generate a return expression directly into R6, avoiding unnecessary MOVE R0, R6
   * instructions.
   *
   ...
   * </ul>
   *
   * <p>Navigates through single-child expression wrappers until reaching a {@code <primarni_izraz>}
   * with a BROJ terminal, or a unary minus followed by a literal.
   *
    ...
    while (true) {
      ...
      if ("<unarni_izraz>".equals(node.symbol()) && children.size() == 2) {
        ...
        if (first instanceof hr.fer.ppj.semantics.tree.NonTerminalNode unaryOp
          ...
          for (ParseNode opChild : unaryOp.children()) {
            if (opChild instanceof hr.fer.ppj.semantics.tree.TerminalNode terminal
                && "MINUS".equals(terminal.symbol())) {
              isNegative = true;
      ...
      if ("<primarni_izraz>".equals(node.symbol())) {
        for (ParseNode child : children) {
          if (child instanceof hr.fer.ppj.semantics.tree.TerminalNode terminal
              && "BROJ".equals(terminal.symbol())) {
   ...
   *
   * <p>For struct returns, the caller provides the address of a result buffer in R2. This method
   * copies the struct from the return expression to that memory location.
   *
   * <p><b>Calling Convention:</b>
   *
   * <ul>
   *   <li>Caller sets R2 = address of result buffer before CALL
   *   <li>Callee copies struct from return expression to [R2]
   *   <li>R6 is unused for struct returns
   * </ul>
   *
  ...
  private void generateStructReturn(NonTerminalNode expression) {
    ...
    if (!(strippedType instanceof StructType structType)) {
      throw new IllegalStateException(
          "Struct return but expression is not a struct type: " + strippedType);
    }
   ...
   *
   * <p>This is a fallback method used when a return statement is encountered but no function exit
   * label is available.
   */
