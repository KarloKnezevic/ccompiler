 ...
 *
 * <p>This class handles the generation of code for function calls, implementing the <b>function
 * call code generation algorithm</b> that translates C function calls into FRISC assembly following
 ...
 *
 * <ol>
 *   <li><b>Function Name Extraction:</b> Extract function name from function expression (must be a
 *       simple identifier, function pointers not supported)
 *   <li><b>Argument Evaluation:</b> Evaluate all argument expressions in left-to-right order (for
 *       side effects), but push them in right-to-left order (for calling convention)
 *   <li><b>Array Argument Handling:</b> For array arguments, pass the address instead of the value
 *       (C array decay to pointer semantics)
 *   <li><b>Function Call:</b> Emit CALL instruction to function label
 *   <li><b>Stack Cleanup:</b> Remove arguments from stack (caller cleans up)
 *   <li><b>Result Handling:</b> Move return value from R6 to R0
 * </ol>
 *
 ...
 *       <ul>
 *         <li>Last argument pushed first (at highest address)
 *         <li>First argument pushed last (at lowest address, top of stack)
 *       </ul>
 *   <li><b>Return Value:</b> Function returns value in register R6
 *   <li><b>Stack Cleanup:</b> Caller removes arguments from stack after call
 *   <li><b>Register Preservation:</b> Callee may use R0-R4, must preserve R5 (FP) and R7 (SP)
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li><b>Array Detection:</b> Check if argument is a simple array variable
 *   <li><b>Address Generation:</b> Generate code to compute array base address:
 *       <ul>
 *         <li>Global arrays: {@code MOVE G_ARRAY, R0}
 *         <li>Local arrays: {@code MOVE R5, R0; ADD R0, -offset, R0}
 *         <li>Array parameters: {@code LOAD R0, (R5+offset)} (load pointer value)
 *       </ul>
 *   <li><b>Address Push:</b> Push the address (not the array contents) onto the stack
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li>Evaluate arguments left-to-right (preserves side effect order)
 *   <li>Store each result temporarily (on stack or in register)
 *   <li>Push arguments right-to-left (last argument first)
 * </ol>
 *
 * <p>In this implementation, we evaluate and push immediately, which achieves the correct order
 * because we iterate through arguments in reverse order.
 *
 ...
 *
 * <ul>
 *   <li><b>Time Complexity:</b> O(n) where n is the number of arguments (each argument is evaluated
 *       and pushed once)
 *   <li><b>Space Complexity:</b> O(1) - uses only registers and stack space for arguments
 * </ul>
 *
...
public final class FunctionCallGenerator {
   ...
   */
  public void generateFunctionCallWithReturnPointer(
      NonTerminalNode function, NonTerminalNode arguments, String returnPointerRegister) {
    ...
    if (functionName == null) {
      ...
      throw new IllegalStateException(
          "Complex function call (function pointer) not supported in this subset");
    }
    ...
    if (returnsStruct) {
      if (returnPointerRegister != null) {
        ...
        context
            .emitter()
            .emitInstruction(
                "MOVE", returnPointerRegister, "R1", "save struct return buffer address");
      } else {
        // No return pointer provided - this shouldn't happen for struct returns
        throw new IllegalStateException(
            "Struct-returning function call requires return pointer register");
      }
    }
   ...
   *
   * <p>Arguments are pushed onto the stack in right-to-left order (last argument first). Array
   * variables are passed as addresses rather than values. Struct arguments are copied word-by-word
   * onto the stack.
   *
      ...
      if (strippedArgType instanceof StructType structType) {
        ...
        int structSize =
            StructSizeCalculator.calculateStructSize(
                structType, arraySizes, nestedStructArraySizes);
        structArgumentGenerator.generateStructArgument(
            arg, structType, arraySizes, nestedStructArraySizes);
        ...
        if (arrayVarName != null && isArrayVariable(arrayVarName)) {
          ...
          if (baseAddress.startsWith("(G_")) {
            ...
            context
                .emitter()
                .emitInstruction("MOVE", label, "R0", "load array address " + arrayVarName);
          } else {
            ...
            if (offsetStr != null) {
              ...
              context
                  .emitter()
                  .emitInstruction("ADD", "R0", formattedOffset, "R0", "compute array address");
            } else {
        ...
        context
            .emitter()
            .emitInstruction("PUSH", "R0", null, "push argument " + (i + 1) + " (value)");
  ...
  private void extractArgumentExpressionsRecursive(
      NonTerminalNode node, List<NonTerminalNode> expressions) {
