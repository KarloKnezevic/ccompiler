...
public final class HelpPrinter {
  ...
  public String render() {
    return String.join(System.lineSeparator(),
        ...
        "  java -jar ccompiler run-vm [--trace-vm] [--dump-bytecode] <program.ir>",
        "",
        "Options:",
        ...
        "  --verify-each Run the IR verifier after every optimization pass (debugging aid)",
        "  --trace-ir    (run-ir) Print instruction trace",
        "  --ir-step-limit <N>  (run-ir) Max interpreter steps",
        "  --trace-vm    (run-vm) Print per-dispatch bytecode trace",
        "  --dump-bytecode  (run-vm) Disassemble bytecode instead of running",
        "  --vm-dispatch-limit <N>  (run-vm) Max dispatched bytecode instructions",
