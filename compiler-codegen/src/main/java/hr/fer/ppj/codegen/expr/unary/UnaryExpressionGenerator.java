 ...
 *
 * <ol>
 *   <li><b>Operator Identification:</b> Identify the unary operator (+, -, !, ~) or cast type
 *   <li><b>Operand Evaluation:</b> Evaluate the operand expression (result in R0)
 *   <li><b>Operation Application:</b> Apply the unary operation or type conversion:
 *       <ul>
 *         <li><b>Unary Plus (+):</b> No-op (identity operation)
 *         <li><b>Unary Minus (-):</b> Negation (0 - operand, or direct negative literal)
 *         <li><b>Logical NOT (!):</b> Boolean inversion (0 ↔ 1)
 *         <li><b>Bitwise NOT (~):</b> Bitwise complement (XOR with all 1s)
 *         <li><b>Type Casts:</b> Type conversion operations (see below)
 *       </ul>
 * </ol>
 *
 ...
 *
 * <ol>
 *   <li><b>Literal Optimization:</b> If operand is a literal (int or float), directly emit the
 *       negated value using constant loading
 ...
 *
 * <ol>
 *   <li><b>Zero Check:</b> Compare operand with 0
 *   <li><b>Conditional Assignment:</b>
 *       <ul>
 *         <li>If operand == 0: Result = 1 (true)
 *         <li>If operand != 0: Result = 0 (false)
 *       </ul>
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><b>int → char:</b> Mask lower 8 bits: {@code AND R0, 00FF, R0}
 *   <li><b>char → int:</b> No-op (char is already 32-bit in this implementation)
 *   <li><b>int → float:</b> Call F_I2F helper function (convert to Q16.16)
 *   <li><b>float → int:</b> Call F_F2I helper function (truncate Q16.16 to integer)
 *   <li><b>float → char:</b> First convert float to int, then mask to char
 * </ul>
 *
...
public final class UnaryExpressionGenerator {
   ...
   *
   * <ul>
   *   <li>Single operand (delegates to next level)
   *   <li>Unary operator followed by operand: OP <operand>
   * </ul>
   *
  ...
  public void generateUnaryExpression(NonTerminalNode node) {
    ...
    if (children.size() == 1) {
      ...
      if (first instanceof TerminalNode terminal) {
        ...
        // Structure: <unarni_izraz> -> <unarni_operator> -> MINUS/PLUS/OP_TILDA/OP_NEG +
        // <cast_izraz>
  ...
  public void generateCastExpression(NonTerminalNode node) {
    ...
    // Structure: children[0] = L_ZAGRADA, children[1] = <ime_tipa>, children[2] = D_ZAGRADA,
    // children[3] = <cast_izraz>
    ...
    if (targetType == PrimitiveType.CHAR) {
      ...
      // Use hex format for bit mask: 00FF (not %D 255, as this is a bit mask, not a C integer
      // literal)
      ...
      context
          .emitter()
          .emitInstruction("AND", "R0", "00FF", "R0", "cast to char (mask lower 8 bits)");
   ...
   *
   * <ul>
   *   <li>If operand is float (Q16.16): Generate direct negation in Q16.16 format (no F_I2F)
   *   <li>If operand is int: Generate integer negation
   * </ul>
   *
   * <p><b>Optimization:</b> For simple literals (not complex expressions), directly emits the
   * negated value using {@code emitLoadIntConstant} (which handles large negative values
   * correctly).
   *
   * <p><b>FRISC Code Pattern:</b>
   *
   * <ul>
   *   <li>Float literal: Direct Q16.16 negation (already in Q16.16 format)
   *   <li>Integer literal: {@code MOVE %D -value, R0} (or SHL/ADD for large values)
   *   <li>Non-literal: {@code MOVE %D 0, R1; SUB R1, R0, R0} (0 - operand)
   * </ul>
   *
   * <p><b>FRISC Semantics:</b> Uses SUB instruction for runtime negation: {@code SUB R1, R0, R0}
   * computes R0 = R1 - R0 = 0 - operand = -operand. For float operands, this preserves Q16.16
