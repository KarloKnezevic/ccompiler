 ...
 * 
 * <p>This class searches the parse tree to find array size declarations
 * in the form: {@code IDN L_UGL_ZAGRADA BROJ D_UGL_ZAGRADA}.
 * 
 * <p>The extractor navigates through the parse tree structure, looking
 * for external declarations and extracting the size value from array
 * declarators.
 * 
...
public final class ArraySizeExtractor {
    ...
    private int extractArraySizeFromDeclarator(NonTerminalNode node, String variableName) {
        ...
        if (children.size() == 4) {
            ...
            if (first instanceof TerminalNode idToken && "IDN".equals(idToken.symbol()) &&
                second instanceof TerminalNode leftBracket && "L_UGL_ZAGRADA".equals(leftBracket.symbol()) &&
                third instanceof TerminalNode sizeToken && "BROJ".equals(sizeToken.symbol()) &&
                fourth instanceof TerminalNode rightBracket && "D_UGL_ZAGRADA".equals(rightBracket.symbol())) {
