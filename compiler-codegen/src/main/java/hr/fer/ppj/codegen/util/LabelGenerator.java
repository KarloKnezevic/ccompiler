 ...
 *
 * <p>This class ensures that all generated labels are unique and follow consistent naming
 * conventions. It supports different label types:
 *
 * <ul>
 *   <li>Function labels (F_MAIN, F_FOO, etc.)
 *   <li>Global variable labels (G_X, G_ARRAY, etc.)
 *   <li>Control flow labels (L_IF1, L_ELSE1, L_END1, etc.)
 *   <li>Loop labels (L_LOOP1, L_CONTINUE1, L_BREAK1, etc.)
 *   <li>Short-circuit evaluation labels (L_SC_TRUE1, L_SC_FALSE1, etc.)
 * </ul>
 *
 * <p>Labels are generated with sequential numbering to ensure uniqueness within each category. The
 * generator maintains separate counters for different label prefixes to avoid conflicts.
 *
...
public final class LabelGenerator {
   ...
   *
   * <p>Function labels follow the pattern {@code F_<NAME>} where NAME is the uppercased function
   * identifier. Function labels are not numbered since function names are unique by definition.
   *
   ...
   *
   * <p>Global variable labels follow the pattern {@code G_<NAME>} where NAME is the uppercased
   * variable identifier. Global variable labels are not numbered since variable names are unique by
   * definition.
   *
   ...
   *
   * <p>Control flow labels are numbered sequentially to ensure uniqueness. Common prefixes include:
   *
   * <ul>
   *   <li>"L_IF" for if statement labels
   *   <li>"L_ELSE" for else clause labels
   *   <li>"L_END" for end-of-block labels
   *   <li>"L_LOOP" for loop start labels
   *   <li>"L_CONTINUE" for continue target labels
   *   <li>"L_BREAK" for break target labels
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>ifLabel - for the condition check
   *   <li>elseLabel - for the else clause
   *   <li>endLabel - for the end of the if-else block
   * </ul>
   *
  ...
  public IfLabelSet generateIfLabels() {
    ...
    return new IfLabelSet("L_IF" + suffix, "L_ELSE" + suffix, "L_END" + suffix);
  }
   ...
   *
   * <ul>
   *   <li>loopLabel - for the loop start/condition check
   *   <li>continueLabel - for continue statements
   *   <li>breakLabel - for break statements
   * </ul>
   *
  ...
  public LoopLabelSet generateLoopLabels() {
    ...
    return new LoopLabelSet("L_LOOP" + suffix, "L_CONTINUE" + suffix, "L_BREAK" + suffix);
  }
   ...
   *
   * <ul>
   *   <li>trueLabel - for the true branch
   *   <li>falseLabel - for the false branch
   *   <li>endLabel - for the end of the evaluation
   * </ul>
   *
  ...
  public ShortCircuitLabelSet generateShortCircuitLabels() {
    ...
    return new ShortCircuitLabelSet(
        "L_SC_TRUE" + suffix, "L_SC_FALSE" + suffix, "L_SC_END" + suffix);
  }
